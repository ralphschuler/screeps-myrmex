...
resource-manifest container migration, extension, lab, reserve-link, spawn, or tower evacuation is
the sole active-work exception: its original deadline and exact source/replacement/amount/baseline
identity may continue through the existing funded V3 Logistics path. A stale lab reuses only known
V1 committed positions as narrow assignment evidence and retains every current quiescent or exact
reaction/boost-handoff, storage precedence, terminal no-send, Store, cooldown, funding, mixed-flow,
and lease gate. Its source/replacement endpoint and applicable terminal-send suppression survive
optional denial; active/pending Industry work may continue exact evacuation but cannot clear the
term. Fresh exact delivery plus complete work retirement clears only a quiescent lab term command-
free before the separate revision handoff. A source-unspecific manifest contains one non-energy row
