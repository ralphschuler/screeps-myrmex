...
to build planned geometry. Current owned migration uses direct `Structure.destroy` only after the
existing safety gates. Unboosted `Creep.dismantle` recovers at most 0.25 energy per active `WORK`
action before integer flooring, versus two source energy from that part's harvest action; it also
delays committed geometry and consumes the creep's primary-action slot. MYRMEX therefore rejects
that command until a later funded policy proves positive full-cost value without weakening
replacement, access, reserve, threat, or rollback evidence. Phase 2 may use spare extension
allowance to build committed capacity, then remove one active obsolete extension only after the room
reaches full allowance with exactly allowance minus one active extensions on committed geometry and
an exact owned replacement remains. A stocked target first persists one bounded evacuation, reserves
exact source/replacement capacity in the sole logistics graph, and suppresses ordinary refill of
both targets during acquisition. Empty-source suppression continues through delivery. Removal waits
for fresh empty-source, delivered-replacement, and retired-flow evidence. Current site headroom is
required; the executor rechecks target stock and replacement identity; only fresh observation proves
removal. Stored structures other than this one extension, defensive, critical, foreign, unknown, and
multiply occupied structures remain excluded. General migration remains fail-closed. One
source-container exception removes an unshared, unselected container beside exactly one source only
while a different exact committed source-container remains the selected reachable service for that
source. An empty target is directly eligible; a stocked target persists one bounded source-specific
handoff and reuses the exact resource-manifest logistics path before removal. The stationary mining
identity and work position do not change; selected, sole, ambiguous, or replacementless containers
remain. A separate general-container exception first spends spare allowance on committed non-service
capacity, then persists one bounded target handoff. An empty target suppresses refill and waits for
every active logistics endpoint naming it to retire. An exact energy-only target whose amount fits
the replacement additionally uses the sole funded logistics flow. A target containing one non-energy
...
convergence resumes no earlier than the next tick. One exact successful stale construction-site
...
issues no retry. It may atomically clear the receipt with one sole container, extension, lab,
reserve-link, spawn, storage, terminal, or tower evacuation/migration only when exact type, target,
replacement, and receipt-within-interval evidence match; the old removal attempt already proves that
handoff reached its command boundary, while retained-target presence proves no removal effect.
...
resource-manifest container migration, extension, lab, reserve-link, spawn, storage, terminal, or
tower evacuation is the sole active-work exception: its original deadline and exact
source/replacement/ amount/baseline identity may continue through the existing funded V3 Logistics
path. A stale lab reuses only known V1 committed positions as narrow assignment evidence and retains
every current quiescent or exact reaction/boost-handoff, storage precedence, terminal no-send,
Store, cooldown, funding, mixed-flow, and lease gate. Its source/replacement endpoint and applicable
terminal-send suppression survive optional denial; active/pending Industry work may continue exact
evacuation but cannot clear the term. Fresh exact delivery plus complete work retirement clears only
a quiescent lab term command- free before the separate revision handoff. A stale terminal accepts
the existing scalar or canonical two-to-eight-row manifest only under fresh exact active
zero-cooldown terminal, active storage, general-purpose Store, current Industry quiescence, safe
colony policy, complete-row funding, and 3,000-unit aggregate bounds. Endpoint and internal-send
suppression survives optional denial. Fresh source emptiness, every exact baseline-plus-amount
storage gain, and complete flow/endpoint retirement clear only the terminal term command-free before
the later revision handoff. A stale storage accepts every existing scalar or canonical
two-to-eight-row form through 6,000 aggregate units while the exact active terminal remains
quiescent and capable. Current rows retain atomic funded Logistics execution. Exact first-batch
source/terminal conservation plus complete prior-work retirement advances only the existing
3,000-unit cursor, keeping suppression continuous and deferring batch two to a later tick. Exact
final source emptiness, every original terminal gain, and retired work clear only the storage term
command-free before the separate handoff. A source-unspecific manifest contains one non-energy row
or two through eight canonical rows with no legacy energy fields; each row retains a distinct funded
flow/budget identity and all share aggregate replacement capacity. One legacy energy form or a
bounded resource manifest may instead retain one exact source identity only while both the target
and a different persisted exact selected replacement service stay adjacent to that source. A
two-to-eight-row source-specific manifest enters lease execution only when every currently
