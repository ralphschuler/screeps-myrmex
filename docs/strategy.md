...
observation proves its exact target ID absent. One completed extension evacuation may remain paired
with that receipt only when exact type, target, replacement, and receipt-within-interval evidence
match; fresh target absence atomically clears both terms. Storage keeps its stronger conservation
boundary; unrelated active, mismatched, failed, present, incomplete, same-tick, or unsafe evidence
remains inert. New layout site/removal planning stops globally for either settlement tick without
