...
[#373](https://github.com/ralphschuler/screeps-myrmex/issues/373) adds one 150-tick continuation for
exactly one resource kind totaling at most 3,000 units, using the same exact terminal, sole funded
V3 Logistics path, removal authority, executor, and receipt. Issue
[#375](https://github.com/ralphschuler/screeps-myrmex/issues/375) composes a canonical bounded
mixed-resource manifest over that same path. Issue
...
active funding, exact delivery, and command-free settlement before the later handoff. Issue
[#423](https://github.com/ralphschuler/screeps-myrmex/issues/423) adds the mutually exclusive
...
3. `industry.publish` first derives one current bounded lab assignment/quiescence view and one
   ...
   primary geometry while current external spawns remain usable. At RCL6+, committed storage
   geometry is similarly restored while the external storage remains usable until its separate
   exact-empty, active-terminal, effective/current/healthy Logistics, work-retirement,
   destination-retirement, and safety removal gates pass. The sole RCL6+ terminal position is also
   restored; removal remains separate and requires one active empty zero-cooldown external target,
   exact active storage, current terminal quiescence, and no unrelated terminal-bound layout or
   Logistics work. One single-resource target or canonical two-to-eight-resource manifest of at most
   3,000 total units may instead persist a 150-tick exact terminal-to-storage handoff before that
...
4. `links.plan` publishes canonical current-layout role classification and funded transfer
   ...
   300-capacity energy-only Store. Current SpawnBroker selections may name neither migration spawn
   nor every idle retained exact spawn. A positive target may persist one exact 150-tick evacuation
   only when the replacement's exact free capacity holds the complete amount. Any unrelated
   assigned/active primary or counterpart endpoint naming either spawn blocks migration. The exact
   evacuation endpoints are allowed during progress but must retire before removal; a newly proposed
   ordinary refill cannot execute before Reconcile and does not become false same-tick activity. Lab
   removal is RCL8-only: all ten owned labs must be observed, exactly nine active labs must occupy
   distinct committed positions, the external target must be active, zero-cooldown, unshared, and
   site-free, and the current industry view must have no commitment, pending attempt, intent,
   staging demand, or demand endpoint. No unrelated active logistics endpoint may name any room lab,
   and deterministic post-removal assignment over the nine exact labs must succeed. An empty target
...
5. On the following tick, runtime composition validates each stocked commitment from fresh
   ...
   unexpired. Only its exact current flows are exempt; owner loss or a future owner rejects every
   prefixed orphan contract/lease. Every terminal or storage manifest row admits atomically before
   and after colony funding; completed rows leave the current group so asymmetric delivery resumes.
   The projection replaces ordinary terminal stock publication, suppresses terminal source/refill
   competition, and blocks every internal send involving the room while the attempt is unexpired.
   Expiry blocks stale evacuation leases and removal but restores ordinary terminal service. Spawn
   work begins only after its staging tick, suppresses obsolete-spawn refill throughout, and
   suppresses replacement refill while source energy remains. The general operational agent pass
   excludes every spawn-evacuation flow plus every V1/V2/V3 lease whose primary/counterpart names a
   currently suppressed migration spawn. After current revalidation, `migration.layout` re-admits,
   submits, funds, and plans only the exact authorized flow. Current SpawnBroker selection or failed
   revalidation therefore excludes stale work from same-tick agent execution; owner-term loss
   cancels, suspends, or fails the orphaned contract through legal ContractLedger transitions. A
   quiescent lab contributes one energy flow to its replacement lab, one mineral flow to the
   Industry-published storage or exact idle terminal, or both distinct flows for a mixed target.
   Either general-purpose mineral sink shares its physical Store's aggregate-capacity key; the
   terminal form also suppresses every internal send from or to the room until the commitment
   clears. The mineral-only and mixed terminal forms may continue under active work for the exact
   reaction or explicit-boost handoff. Mineral-first completion keeps the exact terminal published
   while the remaining energy leg finishes. The obsolete lab source/refill is suppressed. A mixed
...
6. An empty general-container handoff suppresses the obsolete target's ordinary refill and retires
   ...
   in the active committed replacement. A stocked spawn likewise requires exact target emptiness,
   replacement energy at least baseline plus amount, retired exact flow/endpoints, and unchanged
   RCL7/RCL8 geometry, safety, idle, and current SpawnBroker evidence. Replacement refill is allowed
   after acquisition only to restore that committed gain if room-wide spawn energy was consumed. A
   stocked reserve link additionally requires exact target emptiness, exact baseline-plus-amount
   replacement energy, retired flow/endpoints, unchanged reserve roles, zero cooldown, and no
   accepted native link transfer. A stocked terminal requires fresh target emptiness, storage
   exactly baseline plus committed amount, retired exact flow/endpoints, zero cooldown, unchanged
   Industry quiescence, and every existing layout/safety term; internal sends remain suppressed
   while the attempt is active. A stocked storage requires fresh target emptiness, every terminal
   stock row exactly at baseline plus committed amount, retired exact flows and both endpoints,
   current terminal quiescence, and every existing #371 geometry/Logistics/colony safety term. An
   expired attempt restores ordinary endpoint/send service but remains removal-blocking. An
   energy-only lab requires fresh zero energy/mineral, baseline-plus-amount replacement energy,
   retired flow/endpoints, and unchanged quiescence, assignment, post-removal cluster, and safety
   evidence. A mineral-only lab requires fresh emptiness, baseline-plus-amount storage or terminal
   stock, retired flow/endpoints, the same Industry-published destination plus quiescence or exact
   reaction/boost-handoff evidence and cluster/safety evidence, and continued internal-send
   suppression for a terminal. A mixed lab requires both exact destination gains and both flow
   identities plus source, replacement-lab, and storage-or-terminal endpoints to retire; the
   terminal destination is quiescent or an exact active reaction/boost handoff. A durable active
   ...
   replacement-lab and storage-or-terminal gain, retired flow/endpoints, unchanged
   handoff/destination evidence, and no pending attempt. An active explicit boost remains
   removal-blocking until exact next-observation settlement and a following quiescent observation.
   Exact next-observation reaction settlement may continue before target disappearance clears the
   ordinary destroy receipt. Unavailable contract views, capacity loss, consumption, refill, threat,
   timeout, drift, or a projection above 64 flows fail closed without a prefix. Public link-runtime
   ...
   planner authorization and accepts at most one deterministic container, extension, spawn,
   terminal, tower, link, or lab removal after proving current global/room site headroom. The
...
9. Ordinary `layout.reconcile` converts site results and every
   extension/container/spawn/storage/terminal/tower/link/lab destroy result to bounded fingerprinted
...
- at most 64 extension edges, 64 spawn edges, 64 storage edges, 64 terminal edges, 64 tower edges,
  64 reserve-link edges, 64 total lab stock edges, and 64 general-container resource edges, each
  with two nodes; a mixed storage or terminal may consume two through eight corresponding edges, and
  storage/terminal/lab mixed projection overflow rejects the complete migration graph before the
  common logistics caps; any merged optional-demand overflow preserves the non-suppressed observed
  graph rather than restoring durably suppressed migration endpoints or displacing other survival
  nodes;
...
Issue #419's production-runtime evidence starts from one otherwise-quiescent V25 stale record with a
300-energy external reserve link, one exact reserve replacement, five exact committed links, and
complete known-V1 source/hub/controller/reserve-role evidence. Every persisted source-service
identity remains unique and adjacent to its fresh exact source. The existing `optional-growth`
budget, Logistics/V3 contract, allocator, lease agent, action arbiter, and creep executors remain
the only resource path. Both endpoints stay outside native link proposals and ordinary refill even
when optional continuation is denied. Production runtime evidence removes an existing acquire lease
in the same tick that replacement cooldown invalidates the current projection. Fresh source-empty,
exact 300-energy replacement, zero cooldown, unchanged roles, endpoint/flow retirement, and native-
slot exclusion clear only the term command-free; the source-defined handoff follows on a later tick
under bounded RCL8 infrastructure recovery. Warm, module-reset, and reversed-structure variants
produce byte-equivalent owner, planning, and zero-command outcomes. Pure controls reject unknown
...
Issue #423's production-runtime evidence starts from one otherwise-quiescent V25 stale record with a
singleton `U` manifest, 50 units at the general-container target, a 10-unit replacement baseline,
and two non-foreign 2,000-capacity general-purpose Stores. The existing `optional-growth` budget,
Logistics/V3 contract, allocator, lease agent, action arbiter, and creep executors remain the only
resource path. Target source/refill and replacement refill suppression persists when threat denies
optional continuation; active funding, exact resource-qualified flow, current Store identity, and
safe policy gate stale lease execution. Fresh target emptiness, exact 60-unit replacement stock, and
complete flow/source/replacement endpoint retirement clear only `containerMigration` command-free;
the revision handoff follows on a later tick. Warm, module-reset, and reversed-structure variants
produce byte-equivalent owner, planning, and zero-layout-command outcomes. Focused controls reject
current revisions, energy-only or multi-row manifests, source identities, legacy energy fields,
another active term, malformed/capacity-drifted Stores, active work, refill/consumption drift,
threat, expiry, missing funding, and wrong flow identity. No schema, authority, executor, command,
or persistent field is added.

Issue #425's production-runtime evidence starts from one otherwise-quiescent V25 stale record with a
canonical `U`/energy manifest, 50 units of each resource at the target, exact 10/20-unit replacement
baselines, and two non-foreign 2,000-capacity general-purpose Stores. Both existing resource-
qualified `optional-growth` budgets and Logistics/V3 contracts remain active while stale geometry
and removal stay inert. Pure completion evidence rejects asymmetric partial delivery, a missing or
active flow, an active endpoint, and any wrong resource gain; exact 60 `U`, 70 energy, empty target,
every funded inactive flow, and retired endpoints clear only `containerMigration` command-free. The
revision handoff follows on a later tick. Warm, module-reset, reversed-structure, and reversed-
resource variants produce byte-equivalent owner, planning, and zero-layout-command outcomes. An
asymmetric delivered-row observation across module reset and fact reorder retains only the remaining
resource's contract without duplicating completed work. Parameterized `U` and energy rows each prove
that unsafe policy removes their existing stale lease before another withdraw/transfer. Existing
...
also excludes an existing exact acquire lease immediately when unsafe policy withdraws its current
flow and reservation. Fresh source-empty and exact 510 replacement energy plus complete
flow/endpoint retirement clears only that term. Settlement and the following handoff each issue zero
layout commands, and warm, module-reset, and reversed-structure variants are byte-equivalent.
Focused projection and contract-view controls separately prove both-endpoint suppression while the
attempt is unexpired, exact-flow-only admission during in-flight delivery, expiry release, active
flow/endpoints, refill/conservation drift, malformed/duplicate Store evidence, and over-cap input.
Existing V3 executor tests retain withdraw/transfer result coverage. No schema, authority, executor,
or persistent field is added.
...
one source/replacement/interval identity contract. Issue #403 composes the completed terminal-
...
final site, and three active exact spawns produce no further proposal. Its stocked continuation
persists one exact 1–300 amount/baseline under V16, admits one following-tick funded V3 flow,
reserves replacement capacity once, and survives partial transfer plus JSON reset/reordering. Exact
...
one-command/final-site outcome. Empty storage convergence proves one compatible external storage is
retained at RCL4-RCL5 and replaced by committed geometry only when exact terminal continuity exists
at RCL6-RCL8. One exact empty active target passes planner, narrow `storage → terminal` arbitration,
live Store revalidation, V20 receipt reconciliation, fresh disappearance, and final committed-site
reconstruction across reordered observation and JSON reset. Disabled Logistics,
missing/stale/failed/duplicate health, stock, malformed capacity, duplicate/inactive terminal,
current/projected target work, durable lab/terminal destination claims, site conflict, threat, and
live drift fail closed. Its stocked continuation migrates V20 to V21 without invented terms and
persists one exact 1–3,000-unit resource/terminal-baseline record only under current terminal
quiescence and complete aggregate capacity. Tests prove next-tick funded V3 projection, one terminal
capacity reservation, observed/custom endpoint suppression, internal-send suppression, orphan-flow
rejection, partial transfer, JSON reset/reorder identity, exact-delivery contract retirement,
unchanged removal/receipt/disappearance/site reconstruction, expiry service restoration with removal
blocking, and future-owner byte preservation. The mixed continuation migrates V21 to V22 without
invented terms, persists two-to-eight binary-ordered resource/amount/terminal-baseline rows, and
emits distinct flows sharing one terminal aggregate-capacity key. Tests prove reorder/reset
identity, asymmetric partial delivery, complete-current-row admission before and after funding,
malformed/duplicate/over-3,000 rejection, over-64-flow suppression without a prefix, every exact
destination gain, retired manifest work, and the unchanged one-command/final-site outcome. Larger or
malformed stock, source refill, terminal consumption/refill/capacity drift, unrelated endpoints,
missing funding/evidence, threat, timeout, and bounded overflow authorize no unsafe work or command.
Tower replacement-first convergence proves one operational adopted tower makes a committed
replacement site eligible under spare allowance. Its stocked continuation persists one exact amount
only when that active committed replacement begins with 10 energy and has complete capacity; one
externally funded acquire/deliver flow suppresses obsolete-target refill and reserves replacement
capacity. Quiescent lab convergence proves nine labs including one external target produce one
ordinary committed lab site; observing ten labs with nine exact committed positions, matching idle
industry evidence, no logistics endpoint, and a valid post-removal cluster admits one destroy. Exact
...
threat, timeout, or malformed evidence fails closed. Issue
[#349](https://github.com/ralphschuler/screeps-myrmex/issues/349) lets one quiescent mixed target
...
reaction effect blocks removal without discarding evacuation. Issue
[#353](https://github.com/ralphschuler/screeps-myrmex/issues/353) proves the equivalent explicit-
boost path: exact ready evidence publishes the terminal, V14 JSON reconstruction preserves the pair,
and the sole Logistics projection admits both flows atomically through partial delivery. Mineral-
first completion preserves the terminal identity and remaining energy flow. Current or pending boost
work preserves evacuation but blocks removal; exact next-observation body plus 30-mineral/20-energy
settlement and a following quiescent observation expose the existing fresh-delivery and one-command
removal path. The V13 mixed continuation persists both exact amounts, destinations, and baselines,
...
- Official [`Source`](https://docs.screeps.com/api/#Source),
  ...
  static-mining work position; it adds no command. Issues #421, #423, and #425 reuse two exact
  current non-foreign general-purpose container Stores and do not reinterpret source-service
