...
replacement, and tick within the fixed evacuation interval match; both terms clear atomically.
...
target ID. Issue #391 permits one completed extension evacuation to accompany that receipt only when
the receipt type is `extension`, target and replacement IDs match the evacuation, and the receipt
tick lies within its fixed interval. Settlement then atomically removes both terms. Storage retains
its specialized conservation and terminal-continuity proof. Present, same-tick, incomplete, unsafe,
...
- a validated older-algorithm layout record is isolated from every gameplay projection; one exact
  ...
  policy; one completed extension evacuation may clear atomically with that receipt only when exact
  type, target, replacement, and receipt-within-interval evidence match; either settlement is
  ...
  later tick, while unrelated-active, unsafe, blocked, reset, reordered, malformed, failed, foreign,
  storage, or mismatched evidence remains bounded and fail-closed;
