...
const ROOM = "W1N1";
const FIRST_TICK = 90_000;
const MAXIMUM_TICKS = 128;
const SOURCE_ID = "tower-obsolete";
const REPLACEMENT_ID = "tower-replacement";
const HAULER_ID = "tower-migration-hauler";
const EVACUATION_ENERGY = 500;
const MODELED_BUILD_ENERGY_PER_TICK = 100;
const TOWER_BUILD_ENERGY = 5_000;
...
interface VariantSummary {
  ...
  readonly minimumOperationalTowers: number;
...
export async function collectPhase2ActiveStaleTowerConvergenceEvidence() {
  ...
  if (new Set(semanticBytes).size !== 1) {
    throw new Error(`active stale tower convergence drifted: ${canonicalSerialize(semantic)}`);
  }

  return Object.freeze({
    scenario: Object.freeze({
      id: "phase2-active-stale-tower-convergence-v1",
    ...
    authority: Object.freeze({
      ...
      exactDeliveryObserved:
        warm.sourceEnergy === 0 && warm.replacementEnergy === 10 + EVACUATION_ENERGY,
      ...
      destructionObservedNextTick: warm.milestones.siteAt === warm.milestones.destroyAt + 1,
      siteObservedNextTick: warm.milestones.siteObservedAt === warm.milestones.siteAt + 1,
    }),
    ...
    safety: Object.freeze({
      ...
      exactEnergyConserved: warm.sourceEnergy + warm.replacementEnergy === 10 + EVACUATION_ENERGY,
      minimumOperationalTowers: Math.min(
        warm.minimumOperationalTowers,
        reset.minimumOperationalTowers,
        reordered.minimumOperationalTowers,
      ),
...
async function runVariant(variant: Variant): Promise<VariantSummary> {
  ...
  for (let tick = FIRST_TICK + 2; tick < FIRST_TICK + MAXIMUM_TICKS; tick += 1) {
    ...
    if (
      ...
      priorStale?.towerEvacuation !== undefined &&
      currentStale !== undefined &&
      currentStale.towerEvacuation === undefined
    ) {
  ...
  throw new Error(
    `${variant} active stale tower convergence timed out: ${canonicalSerialize({
      commands: world.commands(),
      evacuationSettledAt,
      exactTowers: world.exactTowerCount(),
      flowFundedAt,
      flowRetiredAt,
      handoffAt,
...
function validatedSummary(
  ...
  const sourceEnergy = world.towerEnergy(SOURCE_ID);
  const replacementEnergy = world.towerEnergy(REPLACEMENT_ID);
  const exactTowers = world.exactTowerCount();
  const summary: VariantSummary = Object.freeze({
    ...
    maximumPersistentBytes,
    minimumOperationalTowers: world.minimumOperationalTowers(),
    milestones,
    ...
    totalTowers: world.towerCount(),
  });

  if (sourceEnergy !== 0 || replacementEnergy !== 10 + EVACUATION_ENERGY) {
  ...
  if (
    ...
    milestones.siteAt !== milestones.destroyAt + 1 ||
    milestones.siteObservedAt !== milestones.siteAt + 1 ||
  ...
  if (
    ...
    throw new Error(`${variant} did not reach stable current tower geometry`);
  }
  if (
    world.maximumActiveSites() > 1 ||
    maximumPersistentBytes > 30_000 ||
    world.minimumOperationalTowers() < 1 ||
    world.modeledConstructionEnergy() !== TOWER_BUILD_ENERGY ||
...
function mutableWorld(reverseObservation: boolean) {
  ...
  let active = false;
  let exactTowerPositions: readonly { readonly x: number; readonly y: number }[] = [];
  ...
  let modeledEnergy = 0;
  let maximumSites = 0;
  let minimumOperationalTowers = Number.POSITIVE_INFINITY;
  ...
  const towerStore = (tower: TowerState) => ({
    ...(tower.energy > 0 ? { energy: tower.energy } : {}),
    getCapacity: () => 1_000,
    getFreeCapacity: () => 1_000 - tower.energy,
    getUsedCapacity: (resourceType?: string) =>
      resourceType === undefined || resourceType === "energy" ? tower.energy : 0,
  });
  const towerObject = (tower: TowerState): StructureTower =>
    ({
      destroy: () => {
        if (!active || tower.id !== SOURCE_ID || pendingDestroy) return -7;
        pendingDestroy = true;
        commandRecords.push({ amount: 0, kind: "destroy", targetId: tower.id, tick });
        return 0;
      },
      hits: 3_000,
      hitsMax: 3_000,
      ...
      pos: pos(tower.x, tower.y),
      room: { name: ROOM },
      store: towerStore(tower),
      structureType: "tower",
  ...
  const plannedStructureObject = (placement: LayoutPlacement, index: number): AnyStructure => {
    ...
    if (structureType === "extension") {
      return {
        ...
        store: {
          ...
          getFreeCapacity: () => 0,
          getUsedCapacity: (resourceType?: string) =>
            resourceType === undefined || resourceType === "energy" ? 50 : 0,
        },
    ...
    if (structureType === "link") {
      return { ...base, cooldown: 0, store: emptyStore(800) } as unknown as StructureLink;
    }
    if (structureType === "storage") {
      return { ...base, store: emptyStore(1_000_000) } as unknown as StructureStorage;
    }
  ...
  const hauler = {
    body: [
      ...Array.from({ length: 10 }, () => "carry"),
      ...Array.from({ length: 10 }, () => "move"),
    ].map((type) => ({ hits: 100, type })),
    fatigue: 0,
    hits: 2_000,
    hitsMax: 2_000,
    id: HAULER_ID,
    ...
    store: {
      ...
      getCapacity: () => 500,
      getFreeCapacity: () => 500 - haulerEnergy,
      getUsedCapacity: (resourceType?: string) =>
    ...
    transfer: (target: AnyStoreStructure, resourceType: ResourceConstant, amount?: number) => {
      const replacement = towers.find(({ id }) => id === target.id);
    ...
    withdraw: (target: AnyStoreStructure, resourceType: ResourceConstant, amount?: number) => {
      const obsolete = towers.find(({ id }) => id === target.id);
      if (obsolete === undefined || resourceType !== "energy" || target.id !== SOURCE_ID) return -7;
      const withdrawn = Math.min(amount ?? obsolete.energy, obsolete.energy, 500 - haulerEnergy);
  ...
  const support = {
    body: ["work", "move"].map((type) => ({ hits: 100, type })),
    fatigue: 0,
    hits: 200,
    hitsMax: 200,
    id: "support-worker",
  ...
  const applyDeferredEffects = () => {
    if (!active) return;
    if (pendingDestroy) {
      towers = towers.filter(({ id }) => id !== SOURCE_ID);
    ...
    if (site !== null) {
      const amount = Math.min(MODELED_BUILD_ENERGY_PER_TICK, TOWER_BUILD_ENERGY - site.progress);
      site.progress += amount;
      modeledEnergy += amount;
      commandRecords.push({ amount, kind: "build-modeled", targetId: site.id, tick });
      if (site.progress === TOWER_BUILD_ENERGY) {
        towers.push({
          id: `tower-built-${String(site.x)}-${String(site.y)}`,
          x: site.x,
          y: site.y,
          energy: 0,
        });
    ...
    minimumOperationalTowers = Math.min(
      minimumOperationalTowers,
      towers.filter(({ energy }) => energy >= 10).length,
    );
    if (isStable()) stableTick ??= tick;
  };

  const structures = () => [spawn, ...plannedStructures, ...towers.map(towerObject)];
  const room = {
    controller,
    createConstructionSite: (x: number, y: number, structureType: BuildableStructureConstant) => {
      if (!active || structureType !== "tower" || pendingSite !== null || site !== null) return -8;
      pendingSite = { id: `site-tower-${String(x)}-${String(y)}`, x, y };
  ...
  const siteObject = (): ConstructionSite => {
    ...
    return {
      ...
      progressTotal: TOWER_BUILD_ENERGY,
      structureType: "tower",
  ...
  function exactTowerCount(): number {
    const exact = new Set(exactTowerPositions.map(({ x, y }) => `${String(x)}:${String(y)}`));
    return towers.filter(({ x, y }) => exact.has(`${String(x)}:${String(y)}`)).length;
  }
  ...
  return {
    ...
    towerEnergy: (id: string) => towers.find((tower) => tower.id === id)?.energy ?? 0,
    ...
    game: (nextTick: number): RuntimeGame => {
      if (nextTick <= tick) throw new Error("active stale tower ticks must increase");
      tick = nextTick;
    ...
    minimumOperationalTowers: () => minimumOperationalTowers,
    ...
    seedGeometry: (
      ...
      exactTowerPositions = positions;
      if (positions.length !== 2) throw new Error("expected two RCL5 tower positions");
      const replacement = positions[0];
      if (replacement === undefined) throw new Error("expected exact tower replacement");
      const occupied = new Set(
        placements.map(
          ({ pos: placementPos }) => `${String(placementPos.x)}:${String(placementPos.y)}`,
        ),
      );
      const creepBlocked = new Set(
        placements
          .filter(
            ({ structureType }) =>
      ...
      towers = [
        ...
        { energy: EVACUATION_ENERGY, id: SOURCE_ID, x: external.x, y: external.y },
      ];
      minimumOperationalTowers = 1;
      plannedStructures = [...placements, ...(commitment.sourceServices ?? [])]
        .filter(
          ({ minimumRcl, structureType }) =>
            minimumRcl <= 5 && structureType !== "tower" && structureType !== "spawn",
        )
...
function seedActiveStaleEvacuation(
  ...
  const unlocks = COLONY_RCL_POLICY_TABLE.find(({ level }) => level === 5)?.unlocks;
  if (current === undefined || unlocks === undefined)
    throw new Error("current RCL5 layout unavailable");
  ...
  if (placements === null) throw new Error("current RCL5 layout could not be reconstructed");
  const towerPositions = placements
    .filter(
      ({ layer, minimumRcl, structureType }) =>
        layer === "primary" && minimumRcl <= 5 && structureType === "tower",
    )
  ...
  world.seedGeometry(current, towerPositions, placements);
  const flowId = layoutTowerEvacuationFlowId(ROOM, evacuation);
  const budgetIssuer = layoutTowerEvacuationBudgetIssuer(ROOM, evacuation);
  if (flowId === null || budgetIssuer === null) {
    throw new Error("stale tower flow or budget identity overflowed");
  }
