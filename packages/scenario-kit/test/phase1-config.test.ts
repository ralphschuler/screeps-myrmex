...
describe("Phase 1 runtime config scenario", () => {
  it("retains accepted policy and configured exclusions across rejection and heap reset", () => {
    ...
    expect(
      reset.outcomes.every(({ sourceRevision }) => sourceRevision === "runtime-config-source-v29"),
    ).toBe(true);
        ...
        outcome.gates.every(({ id, enabled, reason, blockedBy }) =>
          id === "phase3.portfolio"
            ? (enabled && reason === "enabled") ||
              (!enabled && reason === "prerequisite-blocked" && blockedBy === "phase2.mature")
            : id === "phase2.mature"
              ? (enabled && reason === "enabled") ||
                (!enabled && reason === "prerequisite-blocked" && blockedBy === "phase2.labs")
              : id === "phase2.labs"
                ? (enabled && reason === "enabled") ||
                  (!enabled && reason === "prerequisite-blocked" && blockedBy === "phase2.industry")
                : id === "phase2.industry"
                  ? (enabled && reason === "enabled") ||
