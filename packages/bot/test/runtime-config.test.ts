...
describe("source feature gates", () => {
  it("activates the mature infrastructure gate only after labs under policy v27", () => {
    ...
    expect(buildRuntimeConfig().features.gates["phase2.logistics"]).toEqual({
      blockedBy: null,
      enabled: true,
      reason: "enabled",
    });
    ...
    expect(buildRuntimeConfig().features.gates["phase2.maintenance"]).toEqual({
      blockedBy: null,
      enabled: true,
      reason: "enabled",
    });
    expect(buildRuntimeConfig().features.gates["phase2.industry"]).toEqual({
      blockedBy: null,
      enabled: true,
      reason: "enabled",
    });
