...
interface GameOptions {
  ...
  readonly controllerPosition?: { readonly x: number; readonly y: number };
  readonly controllerRisk?: boolean;
  readonly reverse?: boolean;
  readonly roomEnergyAvailable?: number;
  readonly roomEnergyCapacityAvailable?: number;
  readonly staleContainerMigration?: {
    readonly replacementEnergy?: number;
    readonly replacementResources?: readonly (readonly [string, number])[];
    readonly resourceType?: string;
    readonly sourceEnergy?: number;
    readonly sourceResources?: readonly (readonly [string, number])[];
  };
  ...
  readonly staleLinkEvacuation?: {
    ...
    readonly replacementEnergy: number;
    readonly sourceEnergy: number;
    readonly storage: { readonly x: number; readonly y: number };
  };
  readonly staleSpawnEvacuation?: {
    readonly replacementEnergy: number;
    readonly sourceEnergy: number;
  };
  readonly staleStorageEvacuation?: {
    readonly terminalResources: readonly (readonly [string, number])[];
    readonly sourceResources: readonly (readonly [string, number])[];
  };
  readonly staleTerminalEvacuation?: {
    readonly replacementResources: readonly (readonly [string, number])[];
    readonly sourceResources: readonly (readonly [string, number])[];
  };
...
describe("stale layout revision runtime handoff (#385/#387/#389/#391/#393/#395/#397/#399/#401/#403/#405/#407/#409/#413/#415/#417/#419/#421/#423/#425/#427/#429/#431/#433/#435/#437)", () => {
  beforeAll(() => {
  ...
  it("continues one stale energy-only container migration through funded logistics", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    runTick({ game: game(100, commands), memory });
    runTick({ game: game(101, commands), memory });
    seedStaleOwner(memory, "energy-container-migration");
  ...
  it("continues one stale singleton non-energy source-container migration through funded logistics", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    runTick({ game: game(100, commands), memory });
    runTick({ game: game(101, commands), memory });
    seedStaleOwner(memory, "source-non-energy-container-migration");
    const sourceServices = layoutsOwner(memory).staleRecords[0]?.sourceServices;
    commands.createConstructionSite.mockClear();
    commands.destroyStructure.mockClear();
    ...
    expect(pending.colony.reservations).toContainEqual(
      expect.objectContaining({
        category: "optional-growth",
        colonyId: ROOM_NAME,
        issuer: layoutContainerMigrationResourceBudgetIssuer(ROOM_NAME, migration, "U"),
        status: "active",
      }),
    );
    expect(
      contracts?.active?.some(
        ({ execution }) => execution?.flowId === flowId && execution.resourceType === "U",
      ),
    ).toBe(true);
    expect(layoutsOwner(memory).staleRecords[0]?.sourceServices).toEqual(sourceServices);
    expect(pending.layout.planning).toEqual([
  ...
  it("continues one stale singleton non-energy container migration through funded logistics", () => {
    ...
    runTick({ game: game(100, commands), memory });
    runTick({ game: game(101, commands), memory });
    seedStaleOwner(memory, "non-energy-container-migration");
    commands.createConstructionSite.mockClear();
    commands.destroyStructure.mockClear();
  ...
  it("continues one stale mixed-resource container migration through funded logistics", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    runTick({ game: game(100, commands), memory });
    runTick({ game: game(101, commands), memory });
    ...
    const resourceTypes = ["U", "energy"] as const;
    const budgetIssuers = resourceTypes.map((resourceType) =>
      layoutContainerMigrationResourceBudgetIssuer(ROOM_NAME, migration, resourceType),
    );
    const flowIds = resourceTypes.map((resourceType) =>
      layoutContainerMigrationResourceFlowId(ROOM_NAME, migration, resourceType),
    );
    ...
    expect(pending.kernel.faults).toEqual([]);
    for (const issuer of budgetIssuers)
      expect(pending.colony.reservations).toContainEqual(
        expect.objectContaining({
          category: "optional-growth",
          colonyId: ROOM_NAME,
  ...
  it("keeps the remaining source-container row funded after partial delivery and reset", async () => {
    ...
    expect(partial.kernel.faults).toEqual([]);
    expect(
      contracts?.active?.some(
        ({ execution }) =>
          execution?.flowId === remainingFlowId && execution.resourceType === "energy",
      ),
    ).toBe(true);
    expect(
  ...
  it("settles delivered stale container stock command-free before later handoff", async () => {
    const forward = await runCompletedStaleContainerMigrationVariant(false, false);
    const reset = await runCompletedStaleContainerMigrationVariant(false, true);
    const reordered = await runCompletedStaleContainerMigrationVariant(true, false);
  ...
  it("settles delivered stale non-energy source-container stock before later handoff", async () => {
    const forward = await runCompletedStaleSourceContainerMigrationVariant(false, false, "U");
    const reset = await runCompletedStaleSourceContainerMigrationVariant(false, true, "U");
    const reordered = await runCompletedStaleSourceContainerMigrationVariant(true, false, "U");
  ...
  it("settles delivered stale non-energy container stock before later handoff", async () => {
    const forward = await runCompletedStaleNonEnergyContainerMigrationVariant(false, false);
    const reset = await runCompletedStaleNonEnergyContainerMigrationVariant(false, true);
    const reordered = await runCompletedStaleNonEnergyContainerMigrationVariant(true, false);
  ...
  it("settles delivered stale mixed container stock before later handoff", async () => {
    const forward = await runCompletedStaleMixedContainerMigrationVariant(false, false);
    const reset = await runCompletedStaleMixedContainerMigrationVariant(false, true);
    const reordered = await runCompletedStaleMixedContainerMigrationVariant(true, false);
  ...
  it("does not continue stale container migration when colony policy is unsafe", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    runTick({ game: game(100, commands), memory });
    runTick({ game: game(101, commands), memory });
    ...
    expect(
      contracts?.active?.some(
        ({ execution, targetId }) =>
          ...
          targetId === migration.replacementId ||
          execution?.counterpartId === migration.targetId ||
          execution?.counterpartId === migration.replacementId,
      ),
    ).not.toBe(true);
    expect(layoutsOwner(memory).staleRecords[0]?.containerMigration).toEqual(migration);
  });

  it("blocks an existing stale container lease when colony policy turns unsafe", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    const baseOptions = {
      roomEnergyAvailable: 350,
  ...
  it("blocks an existing stale non-energy container lease when policy turns unsafe", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    const baseOptions = {
      roomEnergyAvailable: 350,
  ...
  it("blocks an existing stale source-container manifest lease when source-service evidence drifts", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    const baseOptions = {
      roomEnergyAvailable: 350,
    ...
    const migration = layoutsOwner(memory).staleRecords[0]?.containerMigration;
    if (migration === undefined)
      throw new Error("expected stale source-container manifest migration");
    const flowId = layoutContainerMigrationResourceFlowId(ROOM_NAME, migration, "U");
  ...
  it("blocks each existing stale mixed-container lease when policy turns unsafe", () => {
    for (const remainingResource of ["U", "energy"] as const) {
      const commands = commandSpies();
      const memory = {} as Memory;
      const baseOptions: GameOptions = {
        ...
        staleContainerMigration: {
          ...
          sourceResources: [[remainingResource, 50]],
        },
      };
      runTick({ game: game(100, commands), memory });
      runTick({ game: game(101, commands), memory });
      ...
      const migration = layoutsOwner(memory).staleRecords[0]?.containerMigration;
      if (migration === undefined) throw new Error("expected stale mixed container migration");
      const flowId = layoutContainerMigrationResourceFlowId(
        ROOM_NAME,
        migration,
        remainingResource,
      );
      if (flowId === null) throw new Error("expected bounded mixed container flow identity");
  ...
  it.each(STORAGE_EVACUATION_VARIANTS)(
    "continues one stale storage $name evacuation through the exact funded current-batch V3 flow set",
    ({ evacuation }) => {
      const commands = commandSpies();
      const memory = {} as Memory;
      const base = {
      ...
      runTick({ game: game(100, commands, base), memory });
      seedStaleStorageEvacuation(memory, evacuation);
      commands.createConstructionSite.mockClear();
      commands.destroyStructure.mockClear();
      ...
      const pending = runTick({
        game: game(201, commands, {
          ...base,
          staleStorageEvacuation: storageEvacuationObservation(evacuation, "current"),
          workerPosition: { x: 40, y: 37 },
        }),
        memory,
      });
      const flowIds = layoutStorageEvacuationFlowIds(ROOM_NAME, evacuation);
      ...
      expect(layoutsOwner(memory).records).toEqual([]);
      expect(layoutsOwner(memory).staleRecords[0]?.storageEvacuation).toEqual(evacuation);
      expect(commands.createConstructionSite).not.toHaveBeenCalled();
      expect(commands.destroyStructure).not.toHaveBeenCalled();
  ...
  it("advances one stale two-batch storage cursor command-free before publishing batch two", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    const evacuation = {
      ...STORAGE_EVACUATION_VARIANTS[2].evacuation,
      settledAmount: 0,
    } as const satisfies LayoutStorageEvacuation;
    const advanced = { ...evacuation, settledAmount: 3_000 } as const;
    const base = {
      controllerLevel: 6,
      roomEnergyAvailable: 2_300,
      roomEnergyCapacityAvailable: 2_300,
    ...
    const delivered = {
      ...base,
      staleStorageEvacuation: storageEvacuationObservation(evacuation, "first-batch-delivered"),
    } as const;
    const firstFlowIds = layoutStorageEvacuationFlowIds(ROOM_NAME, evacuation);
    const secondFlowIds = layoutStorageEvacuationFlowIds(ROOM_NAME, advanced);
    if (firstFlowIds === null || secondFlowIds === null)
      throw new Error("expected bounded storage batch identities");
    expect(secondFlowIds.some((flowId) => firstFlowIds.includes(flowId))).toBe(false);
    commands.createConstructionSite.mockClear();
    commands.destroyStructure.mockClear();
    commands.sendTerminal.mockClear();

    const cursorTick = 201;
    const cursor = runTick({ game: game(cursorTick, commands, delivered), memory });
    expect(cursor.kernel.faults).toEqual([]);
    expect(layoutsOwner(memory).staleRecords[0]?.storageEvacuation).toEqual(advanced);
    expect(commands.createConstructionSite).not.toHaveBeenCalled();
    expect(commands.destroyStructure).not.toHaveBeenCalled();
    expect(commands.sendTerminal).not.toHaveBeenCalled();
    const contractsAtCursor = contractsView(memory) as
      ...
      | undefined;
    expect(
      secondFlowIds.some((flowId) =>
        contractsAtCursor?.active?.some(({ execution }) => execution?.flowId === flowId),
      ),
    ).toBe(false);
  ...
  it.each(STORAGE_EVACUATION_VARIANTS)(
    "settles delivered stale storage $name stock command-free before a later handoff",
    async ({ evacuation }) => {
      const forward = await runCompletedStaleStorageEvacuationVariant(evacuation, false, false);
      const reset = await runCompletedStaleStorageEvacuationVariant(evacuation, false, true);
      const reordered = await runCompletedStaleStorageEvacuationVariant(evacuation, true, false);
      ...
      expect(forward.settlementCommands).toEqual({ create: 0, destroy: 0, send: 0 });
      expect(forward.handoffOwner).toMatchObject({
        records: [
          expect.objectContaining({ algorithmRevision: "owned-room-layout-v2-source-services" }),
        ],
      ...
      expect(forward.handoffCommands).toEqual({ create: 0, destroy: 0, send: 0 });
      expect(reset).toEqual(forward);
      expect(reordered).toEqual(forward);
    },
    30_000,
  ...
  it("revokes an existing stale storage lease under unsafe policy while retaining suppression", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    ...
    seedStaleStorageEvacuation(memory, evacuation);
    const flowIds = layoutStorageEvacuationFlowIds(ROOM_NAME, evacuation);
    if (flowIds === null) throw new Error("expected bounded stale storage identities");
  ...
  it.each(TERMINAL_EVACUATION_VARIANTS)(
    "continues one stale terminal $name evacuation through the exact funded V3 flow set",
    ({ evacuation }) => {
      const commands = commandSpies();
      const memory = {} as Memory;
      const base = {
      ...
      runTick({ game: game(100, commands, base), memory });
      seedStaleTerminalEvacuation(memory, evacuation);
      commands.createConstructionSite.mockClear();
      commands.destroyStructure.mockClear();
      commands.sendTerminal.mockClear();
      ...
      const pending = runTick({
        game: game(201, commands, {
          ...base,
          staleTerminalEvacuation: {
            replacementResources: terminalEvacuationResources(evacuation, "replacement-initial"),
            sourceResources: terminalEvacuationResources(evacuation, "source"),
          },
          workerPosition: { x: 40, y: 37 },
        }),
        memory,
      });
      const flowIds = layoutTerminalEvacuationFlowIds(ROOM_NAME, evacuation);
      ...
      expect(layoutsOwner(memory).records).toEqual([]);
      expect(layoutsOwner(memory).staleRecords[0]?.terminalEvacuation).toEqual(evacuation);
      expect(commands.createConstructionSite).not.toHaveBeenCalled();
      expect(commands.destroyStructure).not.toHaveBeenCalled();
      expect(commands.sendTerminal).not.toHaveBeenCalled();
    },
  ...
  it.each(TERMINAL_EVACUATION_VARIANTS)(
    "settles delivered stale terminal $name stock command-free before a later handoff",
    async ({ evacuation }) => {
      const forward = await runCompletedStaleTerminalEvacuationVariant(evacuation, false, false);
      const reset = await runCompletedStaleTerminalEvacuationVariant(evacuation, false, true);
      const reordered = await runCompletedStaleTerminalEvacuationVariant(evacuation, true, false);
  ...
  it("revokes an existing stale terminal lease under unsafe policy while retaining suppression", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    const evacuation = TERMINAL_EVACUATION_VARIANTS[0].evacuation;
    const base = {
      controllerLevel: 6,
      roomEnergyAvailable: 2_300,
      roomEnergyCapacityAvailable: 2_300,
    ...
    seedStaleTerminalEvacuation(memory, evacuation);
    const flowIds = layoutTerminalEvacuationFlowIds(ROOM_NAME, evacuation);
    if (flowIds === null) throw new Error("expected bounded stale terminal flow identities");
  ...
  it("continues one stale spawn evacuation through the existing funded logistics path", () => {
    ...
    seedStaleOwner(memory, "spawn-evacuation");
    commands.createConstructionSite.mockClear();
    commands.destroyStructure.mockClear();

    const pending = runTick({
      game: game(202, commands, {
        roomEnergyAvailable: 600,
        roomEnergyCapacityAvailable: 900,
        staleSpawnEvacuation: { replacementEnergy: 0, sourceEnergy: 300 },
        staticMiner: true,
      }),
      memory,
    });
    ...
    expect(layoutsOwner(memory).records).toEqual([]);
    expect(layoutsOwner(memory).staleRecords[0]?.spawnEvacuation).toEqual(evacuation);
    expect(commands.createConstructionSite).not.toHaveBeenCalled();
    expect(commands.destroyStructure).not.toHaveBeenCalled();
  });

  it("settles delivered stale spawn stock command-free before a later revision handoff", async () => {
    const forward = await runCompletedStaleSpawnEvacuationVariant(false, false);
    const reset = await runCompletedStaleSpawnEvacuationVariant(false, true);
    const reordered = await runCompletedStaleSpawnEvacuationVariant(true, false);
  ...
  it("blocks an existing stale spawn lease when colony policy turns unsafe", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    const baseOptions = {
      roomEnergyAvailable: 600,
    ...
    runTick({ game: game(101, commands, { staticMiner: true }), memory });
    seedStaleOwner(memory, "spawn-evacuation");
    const evacuation = layoutsOwner(memory).staleRecords[0]?.spawnEvacuation;
    if (evacuation === undefined) throw new Error("expected stale spawn evacuation");
    const flowId = layoutSpawnEvacuationFlowId(ROOM_NAME, evacuation);
  ...
  it("continues one stale reserve-link evacuation without a native transfer", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    const base = {
      controllerLevel: 8,
    ...
    runTick({ game: game(100, commands, base), memory });
    runTick({ game: game(101, commands, base), memory });
    seedStaleOwner(memory, "link-evacuation");
    const linkOptions = staleLinkEvacuationOptions(memory, 300, 0);
    commands.createConstructionSite.mockClear();
    commands.destroyStructure.mockClear();
    commands.transferLinkEnergy.mockClear();

    const pending = runTick({ game: game(202, commands, { ...base, ...linkOptions }), memory });
    const evacuation = layoutsOwner(memory).staleRecords[0]?.linkEvacuation;
    ...
    expect(contracts?.active?.some(({ execution }) => execution?.flowId === flowId)).toBe(true);
    expect(layoutsOwner(memory).records).toEqual([]);
    expect(layoutsOwner(memory).staleRecords[0]?.linkEvacuation).toEqual(evacuation);
    expect(commands.transferLinkEnergy).not.toHaveBeenCalled();
    expect(commands.createConstructionSite).not.toHaveBeenCalled();
    expect(commands.destroyStructure).not.toHaveBeenCalled();
  });

  it("blocks an existing stale reserve-link lease when current link safety drifts", () => {
    const commands = commandSpies();
    const memory = {} as Memory;
    const base = {
      controllerLevel: 8,
    ...
    runTick({ game: game(101, commands, base), memory });
    seedStaleOwner(memory, "link-evacuation");
    const safe = staleLinkEvacuationOptions(memory, 300, 0);
    const evacuation = layoutsOwner(memory).staleRecords[0]?.linkEvacuation;
    if (evacuation === undefined || safe.staleLinkEvacuation === undefined)
      throw new Error("expected stale reserve-link evacuation");
    const flowId = layoutLinkEvacuationFlowId(ROOM_NAME, evacuation);
    ...
    const drifted = {
      ...safe,
      staleLinkEvacuation: {
        ...safe.staleLinkEvacuation,
        exactLinks: safe.staleLinkEvacuation.exactLinks.map((link) =>
          link.id === "link-replacement" ? { ...link, cooldown: 1 } : link,
        ),
      },
    };
  ...
  it("settles delivered stale reserve-link stock command-free before a later handoff", async () => {
    const forward = await runCompletedStaleLinkEvacuationVariant(false, false);
    const reset = await runCompletedStaleLinkEvacuationVariant(false, true);
    const reordered = await runCompletedStaleLinkEvacuationVariant(true, false);
    ...
    expect(forward.settlementOwner.staleRecords[0]?.linkEvacuation).toBeUndefined();
    expect(forward.settlementCommands).toEqual({ create: 0, destroy: 0, link: 0 });
    expect(forward.handoffPlanning).toEqual([
      expect.objectContaining({ blocker: null, roomName: ROOM_NAME, status: "handoff" }),
    ]);
    expect(forward.handoffOwner).toMatchObject({
    ...
    expect(forward.handoffCommands).toEqual({ create: 0, destroy: 0, link: 0 });
    expect(reset).toEqual(forward);
    expect(reordered).toEqual(forward);
  });
...
function seedStaleStorageEvacuation(memory: Memory, evacuation: LayoutStorageEvacuation): void {
  seedStaleOwner(memory, "storage-evacuation");
  const owner = layoutsOwner(memory);
  memory.myrmex = {
    ...memory.myrmex,
    layouts: {
      ...owner,
      staleRecords: owner.staleRecords.map((record) =>
        record.roomName === ROOM_NAME ? { ...record, storageEvacuation: evacuation } : record,
      ),
    },
  } as unknown as NonNullable<Memory["myrmex"]>;
}

function storageEvacuationObservation(
  evacuation: LayoutStorageEvacuation,
  state: "current" | "delivered" | "first-batch-delivered",
): NonNullable<GameOptions["staleStorageEvacuation"]> {
  const resources = layoutStorageEvacuationResources(evacuation);
  const totalDelivered =
    state === "delivered"
      ? resources.reduce((total, [, amount]) => total + amount, 0)
      : state === "first-batch-delivered"
        ? 3_000
        : "settledAmount" in evacuation
          ? evacuation.settledAmount
          : 0;
  let remainingDelivered = totalDelivered;
  const terminalResources: (readonly [string, number])[] = [];
  const sourceResources: (readonly [string, number])[] = [];
  for (const [resourceType, amount, terminalInitialAmount] of resources) {
    const delivered = Math.min(amount, remainingDelivered);
    remainingDelivered -= delivered;
    const terminalAmount = terminalInitialAmount + delivered;
    const sourceAmount = amount - delivered;
    if (terminalAmount > 0) terminalResources.push([resourceType, terminalAmount]);
    if (sourceAmount > 0) sourceResources.push([resourceType, sourceAmount]);
  }
  return { sourceResources, terminalResources };
}

function seedStaleTerminalEvacuation(memory: Memory, evacuation: LayoutTerminalEvacuation): void {
  seedStaleOwner(memory, "terminal-evacuation");
  const owner = layoutsOwner(memory);
  memory.myrmex = {
    ...memory.myrmex,
    layouts: {
      ...owner,
      staleRecords: owner.staleRecords.map((record) =>
        record.roomName === ROOM_NAME ? { ...record, terminalEvacuation: evacuation } : record,
      ),
    },
  } as unknown as NonNullable<Memory["myrmex"]>;
}

function terminalEvacuationResources(
  evacuation: LayoutTerminalEvacuation,
  location: "replacement-delivered" | "replacement-initial" | "source",
): readonly (readonly [string, number])[] {
  if ("resourceManifest" in evacuation)
    return evacuation.resourceManifest.map(([resourceType, amount, replacementInitialAmount]) => [
      resourceType,
      location === "source"
        ? amount
        : replacementInitialAmount + (location === "replacement-delivered" ? amount : 0),
    ]);
  return [
    [
      ...
      location === "source"
        ? evacuation.amount
        : evacuation.replacementInitialAmount +
          (location === "replacement-delivered" ? evacuation.amount : 0),
    ],
  ];
}
...
function seedStaleOwner(memory: Memory, active: StaleActiveEvidence, roomName = ROOM_NAME): void {
  ...
  const staleRecord = {
    ...
    ...(active === "container-migration" || completedContainerMigration
      ...
      : active === "energy-container-migration"
        ? {
            containerMigration: {
              energyAmount: 50,
              expiresAt: 350,
    ...
    ...(active === "lab-evacuation" || completedLabEvacuation
      ...
      : {}),
    ...(active === "link-evacuation" || completedLinkEvacuation
      ? {
          linkEvacuation: {
            amount: completedLinkEvacuation ? 800 : 300,
            expiresAt: completedLinkEvacuation ? 250 : 350,
            replacementId: "link-replacement",
            replacementInitialEnergy: 0,
            sourceId: "link-obsolete",
            startedAt: completedLinkEvacuation ? 100 : 200,
          },
        }
      : {}),
    ...(active === "spawn-evacuation" || completedSpawnEvacuation
      ? {
          spawnEvacuation: {
            amount: 300,
            expiresAt: completedSpawnEvacuation ? 250 : 350,
            replacementId: "spawn-replacement",
            replacementInitialEnergy: 0,
            sourceId: "spawn-obsolete",
            startedAt: completedSpawnEvacuation ? 100 : 200,
          },
        }
      : {}),
    ...(active === "storage-evacuation" || completedStorageEvacuation
      ? {
          storageEvacuation: {
            amount: 100,
            expiresAt: completedStorageEvacuation ? 250 : 350,
            resourceType: "energy",
            sourceId: "storage-obsolete",
            startedAt: completedStorageEvacuation ? 100 : 200,
            terminalId: "storage-replacement",
            terminalInitialAmount: 50,
          },
        }
      : {}),
    ...(active === "terminal-evacuation" || completedTerminalEvacuation
      ? {
          terminalEvacuation: {
            amount: 100,
            expiresAt: completedTerminalEvacuation ? 250 : 350,
            replacementId: "terminal-replacement",
            replacementInitialAmount: 0,
            resourceType: "energy",
            sourceId: "terminal-obsolete",
            startedAt: completedTerminalEvacuation ? 100 : 200,
          },
        }
      : {}),
...
async function runCompletedStaleContainerMigrationVariant(reverse: boolean, reset: boolean) {
  const commands = commandSpies();
  let memory = {} as Memory;
  let executeTick = runTick;
...
async function runCompletedStaleSourceContainerMigrationVariant(
  reverse: boolean,
  reset: boolean,
  resourceType?: "U",
) {
...
async function runCompletedStaleNonEnergyContainerMigrationVariant(
  reverse: boolean,
  reset: boolean,
) {
  const commands = commandSpies();
...
async function runCompletedStaleStorageEvacuationVariant(
  evacuation: LayoutStorageEvacuation,
  reverse: boolean,
  reset: boolean,
) {
  ...
  executeTick({ game: game(100, commands, { ...base, reverse }), memory });
  seedStaleStorageEvacuation(memory, evacuation);
  if (reset) {
    memory = JSON.parse(JSON.stringify(memory)) as Memory;
    vi.resetModules();
  ...
  commands.sendTerminal.mockClear();
  const delivered = {
    staleStorageEvacuation: storageEvacuationObservation(evacuation, "delivered"),
  } as const;

  const settlement = executeTick({
    game: game(201, commands, { ...base, ...delivered, reverse }),
    memory,
  });
  const settlementOwner = layoutsOwner(memory);
  const settlementCommands = {
  ...
  const handoff = executeTick({
    game: game(202, commands, { ...base, ...delivered, reverse }),
    memory,
  });
  return {
    handoffCommands: {
...
async function runCompletedStaleTerminalEvacuationVariant(
  evacuation: LayoutTerminalEvacuation,
  reverse: boolean,
  reset: boolean,
) {
  ...
  const delivered = {
    staleTerminalEvacuation: {
      replacementResources: terminalEvacuationResources(evacuation, "replacement-delivered"),
      sourceResources: [],
    },
  } as const;
...
async function runCompletedStaleSpawnEvacuationVariant(reverse: boolean, reset: boolean) {
  const commands = commandSpies();
  let memory = {} as Memory;
  let executeTick = runTick;
  executeTick({ game: game(100, commands, { reverse, staticMiner: true }), memory });
  executeTick({ game: game(101, commands, { reverse, staticMiner: true }), memory });
  seedStaleOwner(memory, "spawn-evacuation");
  if (reset) {
    memory = JSON.parse(JSON.stringify(memory)) as Memory;
    vi.resetModules();
  ...
  const settlement = executeTick({
    game: game(202, commands, {
      ...
      roomEnergyAvailable: 600,
      roomEnergyCapacityAvailable: 900,
      staleSpawnEvacuation: { replacementEnergy: 300, sourceEnergy: 0 },
      staticMiner: true,
    }),
    memory,
  });
...
async function runCompletedStaleLinkEvacuationVariant(reverse: boolean, reset: boolean) {
  const commands = commandSpies();
  let memory = {} as Memory;
  let executeTick = runTick;
  const base = {
  ...
  executeTick({ game: game(100, commands, { ...base, reverse }), memory });
  executeTick({ game: game(101, commands, { ...base, reverse }), memory });
  seedStaleOwner(memory, "link-evacuation");
  const delivered = staleLinkEvacuationOptions(memory, 0, 300);
  if (reset) {
    memory = JSON.parse(JSON.stringify(memory)) as Memory;
    vi.resetModules();
    executeTick = (await import("../src/runtime/tick")).runTick;
  ...
  const settlementCommands = {
    create: commands.createConstructionSite.mock.calls.length,
    destroy: commands.destroyStructure.mock.calls.length,
    link: commands.transferLinkEnergy.mock.calls.length,
  };
  commands.createConstructionSite.mockClear();
  commands.destroyStructure.mockClear();
  commands.transferLinkEnergy.mockClear();
  ...
  const handoff = executeTick({
    game: game(203, commands, { ...base, ...delivered, reverse }),
    memory,
  });
  return {
    handoffCommands: {
...
function staleLinkEvacuationOptions(
  memory: Memory,
  sourceEnergy: number,
  replacementEnergy: number,
): Pick<GameOptions, "controllerPosition" | "sourcePosition" | "staleLinkEvacuation"> {
  const record = layoutsOwner(memory).staleRecords.find(({ roomName }) => roomName === ROOM_NAME);
  const unlocks = COLONY_RCL_POLICY_TABLE.find(({ level }) => level === 8)?.unlocks;
  if (record === undefined || unlocks === undefined) throw new Error("stale RCL8 layout missing");
  const linkPlacements = reconstructStaleLayoutLinkPlacements({
    commitment: record,
    roomName: ROOM_NAME,
    unlocks,
  });
  if (linkPlacements === null || linkPlacements.length !== 6)
    throw new Error("known stale link geometry missing");
  const service = record.sourceServices?.find(
    (placement) => placement.service?.kind === "source-container",
  );
  if (service?.service === undefined) throw new Error("stale source service missing");
  const occupied = new Set(linkPlacements.map(({ x, y }) => `${String(x)}:${String(y)}`));
  const around = (origin: (typeof linkPlacements)[number]) =>
    [-2, -1, 0, 1, 2].flatMap((dy) =>
      [-2, -1, 0, 1, 2].flatMap((dx) => {
        const candidate = { roomName: ROOM_NAME, x: origin.x + dx, y: origin.y + dy };
        return candidate.x > 0 &&
          candidate.x < 49 &&
          candidate.y > 0 &&
          candidate.y < 49 &&
          !occupied.has(`${String(candidate.x)}:${String(candidate.y)}`)
          ? [candidate]
          : [];
      }),
    );
  const [sourceAnchor, controllerAnchor, storageAnchor] = linkPlacements;
  if (sourceAnchor === undefined || controllerAnchor === undefined || storageAnchor === undefined)
    throw new Error("stale role anchors missing");
  const sourceCandidates = around(sourceAnchor);
  const controllerCandidates = around(controllerAnchor);
  const storageCandidates = around(storageAnchor);
  for (const sourceService of sourceCandidates) {
    for (const controller of controllerCandidates) {
      for (const storage of storageCandidates) {
        const evidence: LinkLayoutEvidence = {
          algorithmRevision: record.algorithmRevision,
          controller,
          fingerprint: record.fingerprint,
          linkPlacements,
          sourceServices: [{ pos: sourceService, sourceId: service.service.sourceId }],
          storage,
        };
        const anchors = deriveLinkRoleAnchors(evidence);
        const reserves = anchors.filter(({ role }) => role === "reserve");
        if (
          anchors.filter(({ role }) => role === "source").length !== 1 ||
          anchors.filter(({ role }) => role === "controller").length !== 1 ||
          anchors.filter(({ role }) => role === "hub").length !== 1 ||
          reserves.length < 2
        )
          continue;
        const replacement = reserves[0];
        const missing = reserves[reserves.length - 1];
        if (replacement === undefined || missing === undefined || replacement.id === missing.id)
          continue;
        const owner = layoutsOwner(memory);
        memory.myrmex = {
          ...memory.myrmex,
          layouts: {
            ...owner,
            staleRecords: owner.staleRecords.map((stale) =>
              stale.roomName === ROOM_NAME
                ? {
                    ...stale,
                    sourceServices: (stale.sourceServices ?? []).map((placement) =>
                      placement.service?.kind === "source-container"
                        ? { ...placement, pos: sourceService }
                        : placement,
                    ),
                  }
                : stale,
        ...
        const sourcePosition = {
          x: sourceService.x === 48 ? sourceService.x - 1 : sourceService.x + 1,
          y: sourceService.y,
        };
        return {
          controllerPosition: { x: controller.x, y: controller.y },
          sourcePosition,
          staleLinkEvacuation: {
            exactLinks: anchors.flatMap((anchor, index) =>
              anchor.id === missing.id
                ? []
                : [
                    {
                      energy: anchor.id === replacement.id ? replacementEnergy : 0,
                      id:
                        anchor.id === replacement.id
                          ? "link-replacement"
                          : `link-exact-${String(index)}`,
                      x: anchor.pos.x,
                      y: anchor.pos.y,
                    },
                  ],
  ...
  throw new Error("could not derive distinct stale source/controller/hub/reserve roles");
}
...
function game(
  ...
  const source = {
    ...
    id: `source-${roomName}`,
    pos: pos(options.sourcePosition?.x ?? 10, options.sourcePosition?.y ?? 10),
  ...
  const worker = {
    ...
    pos:
      ...
      options.staleStorageEvacuation === undefined &&
      options.staleTerminalEvacuation === undefined &&
      ...
      : Array.from(
          ...
          (_, index) =>
            ({
              ...
              hits: 300,
              hitsMax: 300,
              id: `link-worker-${String(index)}-${roomName}`,
              my: true,
              name: `link-worker-${String(index)}-${roomName}`,
              owner: { username: "Myrmex" },
              pos: index === 0 ? pos(39, 37) : pos(39 - index, 39),
              room: { name: roomName },
              spawning: false,
              store: { getCapacity: () => 50, getFreeCapacity: () => 50, getUsedCapacity: () => 0 },
              transfer: commands.transferEnergy,
  ...
  const controller = {
    ...
    my: true,
    owner: { username: "Myrmex" },
    pos: pos(options.controllerPosition?.x ?? 25, options.controllerPosition?.y ?? 20),
    progress: 0,
    progressTotal: 1_000,
    safeMode: undefined,
    safeModeAvailable: 1,
  ...
  const storageTerminal =
    staleRemovalTargetType === "storage"
      ...
      : null;
  const staleContainerMigration = options.staleContainerMigration;
  const evacuationContainer = (
    id: string,
    x: number,
    inputResources: readonly (readonly [string, number])[],
  ) => {
    const resources = options.reverse ? [...inputResources].reverse() : inputResources;
    ...
    return {
      ...
      room: { name: roomName },
      store: {
        ...Object.fromEntries(resources.filter(([, amount]) => amount > 0)),
        getCapacity: () => 2_000,
        getFreeCapacity: () => 2_000 - usedCapacity,
        getUsedCapacity: (requested?: string) =>
          requested === undefined
            ? usedCapacity
            : (resources.find(([resourceType]) => resourceType === requested)?.[1] ?? 0),
      },
      structureType: "container",
      ticksToDecay: 5_000,
    } as unknown as StructureContainer;
  };
  const staleContainerSourceResources =
    staleContainerMigration?.sourceResources ??
    (staleContainerMigration?.sourceEnergy === undefined
      ? []
      : [
          [
            staleContainerMigration.resourceType ?? "energy",
            staleContainerMigration.sourceEnergy,
          ] as const,
        ]);
  const staleContainerReplacementResources =
    staleContainerMigration?.replacementResources ??
    (staleContainerMigration?.replacementEnergy === undefined
      ? []
      : [
          [
            staleContainerMigration.resourceType ?? "energy",
            staleContainerMigration.replacementEnergy,
          ] as const,
        ]);
  const staleContainerSource =
    staleContainerMigration === undefined
      ? null
      : evacuationContainer("container-obsolete", 40, staleContainerSourceResources);
  const staleContainerReplacement =
    staleContainerMigration === undefined
      ? null
      : evacuationContainer("container-replacement", 41, staleContainerReplacementResources);
  ...
  const staleSpawnEvacuation = options.staleSpawnEvacuation;
  const evacuationSpawn = (id: string, x: number, energy: number) =>
    ({
      destroy: commands.destroyStructure,
      hits: 5_000,
      hitsMax: 5_000,
      ...
      store: {
        ...(energy > 0 ? { energy } : {}),
        getCapacity: () => 300,
        getFreeCapacity: () => 300 - energy,
        getUsedCapacity: () => energy,
      },
      structureType: "spawn",
    }) as unknown as StructureSpawn;
  const staleSpawnSource =
    staleSpawnEvacuation === undefined
      ? null
      : evacuationSpawn("spawn-obsolete", 40, staleSpawnEvacuation.sourceEnergy);
  const staleSpawnReplacement =
    staleSpawnEvacuation === undefined
      ? null
      : evacuationSpawn("spawn-replacement", 41, staleSpawnEvacuation.replacementEnergy);
  ...
  const staleStorageEvacuation = options.staleStorageEvacuation;
  const evacuationInventoryStore = (
    capacity: number,
    inputResources: readonly (readonly [string, number])[],
  ) => {
    const resources = options.reverse ? [...inputResources].reverse() : inputResources;
    const used = resources.reduce((total, [, amount]) => total + amount, 0);
    return {
      ...Object.fromEntries(resources.filter(([, amount]) => amount > 0)),
      getCapacity: () => capacity,
      ...
      getUsedCapacity: (resourceType?: string) =>
        resourceType === undefined
          ? used
          : (resources.find(([type]) => type === resourceType)?.[1] ?? 0),
    };
  };
  const staleStorageSource =
    staleStorageEvacuation === undefined
      ? null
      : ({
          destroy: commands.destroyStructure,
          hits: 10_000,
          ...
          pos: pos(40, 38),
          room: { name: roomName },
          store: evacuationInventoryStore(1_000_000, staleStorageEvacuation.sourceResources),
          structureType: "storage",
        } as unknown as StructureStorage);
  const staleStorageTerminal =
    staleStorageEvacuation === undefined
      ? null
      : ({
          cooldown: 0,
          ...
          room: { name: roomName },
          send: commands.sendTerminal,
          store: evacuationInventoryStore(300_000, staleStorageEvacuation.terminalResources),
          structureType: "terminal",
        } as unknown as StructureTerminal);
  const staleTerminalEvacuation = options.staleTerminalEvacuation;
  const staleTerminalSource =
    staleTerminalEvacuation === undefined
      ? null
      : ({
          cooldown: 0,
          destroy: commands.destroyStructure,
          ...
          room: { name: roomName },
          send: commands.sendTerminal,
          store: evacuationInventoryStore(300_000, staleTerminalEvacuation.sourceResources),
          structureType: "terminal",
        } as unknown as StructureTerminal);
  const staleTerminalReplacement =
    staleTerminalEvacuation === undefined
      ? null
      : ({
          destroy: commands.destroyStructure,
          ...
          pos: pos(41, 38),
          room: { name: roomName },
          store: evacuationInventoryStore(1_000_000, staleTerminalEvacuation.replacementResources),
          structureType: "storage",
        } as unknown as StructureStorage);
  ...
  const evacuationLink = (id: string, x: number, y: number, energy: number, cooldown = 0) =>
    ({
      ...
      structureType: "link",
      transferEnergy: commands.transferLinkEnergy,
    }) as unknown as StructureLink;
  const staleLinkStructures =
    staleLinkEvacuation === undefined
      ? []
      : [
          evacuationLink("link-obsolete", 40, 37, staleLinkEvacuation.sourceEnergy),
          ...staleLinkEvacuation.exactLinks.map(({ cooldown, energy, id, x, y }) =>
            evacuationLink(id, x, y, energy, cooldown),
          ),
        ];
  const staleLinkStorage =
    staleLinkEvacuation === undefined
      ? null
      : ({
          destroy: commands.destroyStructure,
          hits: 10_000,
          hitsMax: 10_000,
          id: "stale-link-storage",
          isActive: () => true,
          my: true,
          owner: { username: "Myrmex" },
          pos: pos(staleLinkEvacuation.storage.x, staleLinkEvacuation.storage.y),
          room: { name: roomName },
          store: {
  ...
  const structures = options.reverse
    ? [
        ...
        ...(staleContainerSource === null ? [] : [staleContainerSource]),
        ...(staleContainerReplacement === null ? [] : [staleContainerReplacement]),
        ...(staleEvacuationSource === null ? [] : [staleEvacuationSource]),
        ...(staleEvacuationReplacement === null ? [] : [staleEvacuationReplacement]),
        ...(staleSpawnSource === null ? [] : [staleSpawnSource]),
        ...
        ...(staleStorageSource === null ? [] : [staleStorageSource]),
        ...(staleStorageTerminal === null ? [] : [staleStorageTerminal]),
        ...(staleTerminalSource === null ? [] : [staleTerminalSource]),
        ...(staleTerminalReplacement === null ? [] : [staleTerminalReplacement]),
        ...staleLinkStructures,
  ...
  const ownedCreeps =
    staticMiner === null || supportWorker === null
      ? [worker, ...linkWorkers]
      : [worker, supportWorker, staticMiner, ...linkWorkers];
    ...
    getObjectById: (id) =>
      id === worker.id
        ...
        : id === supportWorker?.id
          ...
          : id === staticMiner?.id
            ? staticMiner
            : (linkWorkers.find((creep) => creep.id === id) ??
              (id === spawn.id
                ? spawn
                : id === source.id
                  ? source
                  : id === staleContainerSource?.id
                    ? staleContainerSource
                    : id === staleContainerReplacement?.id
                      ? staleContainerReplacement
                      : id === staleEvacuationSource?.id
                        ? staleEvacuationSource
                        : id === staleEvacuationReplacement?.id
                          ? staleEvacuationReplacement
                          : id === staleSpawnSource?.id
                            ? staleSpawnSource
                            : id === staleSpawnReplacement?.id
                              ? staleSpawnReplacement
                              : id === staleTowerSource?.id
                                ...
                                : id === staleTowerReplacement?.id
                                  ...
                                  : (staleLabStructures.find((lab) => lab.id === id) ??
                                    ...
                                    (id === staleStorageSource?.id ? staleStorageSource : null) ??
                                    (id === staleStorageTerminal?.id
                                      ? staleStorageTerminal
                                      : null) ??
                                    (id === staleTerminalSource?.id ? staleTerminalSource : null) ??
                                    (id === staleTerminalReplacement?.id
                                      ? staleTerminalReplacement
                                      : null) ??
