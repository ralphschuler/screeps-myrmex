...
import {
  ...
  composeReactionHandoffFixture,
  labBoostHandoffFixtureWorld,
  labFixtureObjective,
  labHandoffFixtureWorld,
...
describe("ConstructionPlanner", () => {
  ...
  it("evacuates one quiescent lab mineral to an idle terminal before removal", () => {
    ...
    const evacuation = staged.labEvacuation;
    if (evacuation === null) throw new Error("expected terminal lab evacuation");
    let owner = persistLayoutCommitment(emptyLayoutsOwner(), "W1N1", migrationCommitment);
    owner = persistLayoutLabEvacuation(owner, "W1N1", evacuation);
    owner = parseLayoutsOwner(JSON.parse(JSON.stringify(owner))) ?? emptyLayoutsOwner();
    ...
    if (flowIds === null) throw new Error("expected terminal lab flow identity");
    expect(
      planMigration({
    ...
    expect(
      planMigration({
        ...
        colony: partial.colony,
        labEvacuation: evacuation,
        labMigration: { ...partial.labMigration, evacuationTerminalId: null },
        logisticsEvidenceReady: true,
    ...
    if (complete.authorization === null) throw new Error("expected terminal removal authorization");
    const arbitration = arbitrateStructureRemovals({
      authorizations: [complete.authorization],
      limits: STRUCTURE_REMOVAL_LIMITS,
      proposals: complete.proposals,
  ...
  it("evacuates one quiescent lab's mixed stock through an idle terminal atomically", () => {
    const fixture = labTerminalEvacuationFixture(750, 1_000, 10_000, 100, undefined, 500, 250);
    const staged = planMigration({
      activeLogisticsFlowIds: new Set(),
      activeLogisticsTargetIds: new Set(),
      colony: fixture.colony,
    ...
    if (evacuation === null || !("energyAmount" in evacuation))
      throw new Error("expected mixed terminal lab evacuation");
    let owner = persistLayoutCommitment(emptyLayoutsOwner(), "W1N1", migrationCommitment);
    owner = persistLayoutLabEvacuation(owner, "W1N1", evacuation);
    owner = parseLayoutsOwner(JSON.parse(JSON.stringify(owner))) ?? emptyLayoutsOwner();
    const persisted = owner.records[0]?.labEvacuation;
    if (persisted === undefined) throw new Error("expected persisted mixed terminal evacuation");
    const flowIds = layoutLabEvacuationFlowIds("W1N1", persisted);
    if (flowIds === null || flowIds.length !== 2)
      throw new Error("expected two mixed terminal flow identities");

    const following = labTerminalEvacuationFixture(
      750,
      1_000,
      10_000,
      101,
    ...
    const partial = labTerminalEvacuationFixture(
      300,
      1_450,
      10_450,
      102,
    ...
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set(flowIds),
        activeLogisticsTargetIds: new Set(["lab-external", fixture.replacementId, "terminal"]),
        colony: partial.colony,
    ...
    ).toEqual([expect.objectContaining({ targetId: "lab-external" })]);
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set(),
        activeLogisticsTargetIds: new Set(),
        colony: completeFixture.colony,
        labEvacuation: evacuation,
        labMigration: activeBoostLabMigration(completeFixture, ["commitment"]),
        logisticsEvidenceReady: true,
        placements: completeFixture.placements,
        room: completeFixture.room,
      }).proposals,
  ...
  it("evacuates energy from an obsolete lab while a rebound reaction remains active", () => {
    const fixture = labEvacuationFixture(750, 250, 100);
    const activeMigration = activeReactionLabMigration(fixture);
    const staged = planMigration({
      activeLogisticsFlowIds: new Set(),
      activeLogisticsTargetIds: new Set(),
      colony: fixture.colony,
      labMigration: activeMigration,
      logisticsEvidenceReady: true,
      placements: fixture.placements,
      room: fixture.room,
    });
    ...
    const evacuation = staged.labEvacuation;
    if (evacuation === null) throw new Error("expected active-reaction lab evacuation");
    let owner = persistLayoutCommitment(emptyLayoutsOwner(), "W1N1", migrationCommitment);
    owner = persistLayoutLabEvacuation(owner, "W1N1", evacuation);
    owner = parseLayoutsOwner(JSON.parse(JSON.stringify(owner))) ?? emptyLayoutsOwner();
    ...
    const logistics = projectLayoutLabEvacuations({
      existingBudgets: [],
      migrationRooms: [activeReactionLabMigration(following)],
      records: owner.records,
      snapshot: { rooms: [following.room] } as unknown as WorldSnapshot,
      tick: 101,
    });
    const flowId = layoutLabEvacuationFlowId("W1N1", evacuation);
    if (flowId === null) throw new Error("expected active-reaction lab flow identity");
    expect(logistics).toMatchObject({
      authorizedFlowIds: [flowId],
      budgets: [expect.objectContaining({ category: "optional-growth" })],
      demands: { edges: [expect.objectContaining({ id: flowId, maximumAmount: 750 })] },
    });
    const activeFollowing = activeReactionLabMigration(following);
    expect(
      projectLayoutLabEvacuations({
        existingBudgets: [],
        migrationRooms: [
          {
            ...activeFollowing,
            activity: ["commitment", "intent", "pending-attempt"] as const,
          },
        ],
        records: owner.records,
        snapshot: { rooms: [following.room] } as unknown as WorldSnapshot,
      ...
      authorizedFlowIds: [flowId],
      demands: {
        edges: [expect.objectContaining({ id: flowId })],
        suppressedSinkTargetIds: ["lab-external", fixture.replacementId],
        suppressedSourceTargetIds: ["lab-external", fixture.replacementId],
      },
    });
    for (const blockedMigration of [
      {
        ...activeFollowing,
        assignmentHandoff: { ...activeFollowing.assignmentHandoff, status: "pending" as const },
      },
      {
        ...activeFollowing,
        assignmentHandoff: {
          ...activeFollowing.assignmentHandoff,
          layoutFingerprint: "layout-drift",
        },
      },
    ])
      expect(
        projectLayoutLabEvacuations({
          existingBudgets: [],
          migrationRooms: [blockedMigration],
          records: owner.records,
          snapshot: { rooms: [following.room] } as unknown as WorldSnapshot,
          tick: 101,
        }),
      ).toMatchObject({ authorizedFlowIds: [], budgets: [], demands: { edges: [] } });

    const completeFixture = labEvacuationFixture(0, 1_000, 103, fixture.replacementId);
    const completeMigration = activeReactionLabMigration(completeFixture);
    const complete = planMigration({
      activeLogisticsFlowIds: new Set(),
      activeLogisticsTargetIds: new Set(),
      colony: completeFixture.colony,
      labEvacuation: evacuation,
      labMigration: completeMigration,
      logisticsEvidenceReady: true,
      placements: completeFixture.placements,
      room: completeFixture.room,
    });
    expect(complete.proposals).toHaveLength(1);
    expect(complete.proposals[0]?.stableId).toContain("remove-active-reaction-lab-v1");
    expect(complete.proposals[0]?.targetId).toBe("lab-external");
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set(),
        activeLogisticsTargetIds: new Set(),
  ...
  it("evacuates mineral from an obsolete lab while a rebound reaction remains active", () => {
    const fixture = labMineralEvacuationFixture(750, 1_000, 10_000, 100);
    const activeMigration = activeReactionLabMigration(fixture);
    const staged = planMigration({
      activeLogisticsFlowIds: new Set(),
      activeLogisticsTargetIds: new Set(),
    ...
    const reorderedStaged = planMigration({
      activeLogisticsFlowIds: new Set(),
      activeLogisticsTargetIds: new Set(),
      colony: fixture.colony,
    ...
    expect(JSON.stringify(reorderedStaged)).toBe(JSON.stringify(staged));
    const evacuation = staged.labEvacuation;
    if (evacuation === null) throw new Error("expected active-reaction mineral evacuation");
    let owner = persistLayoutCommitment(emptyLayoutsOwner(), "W1N1", migrationCommitment);
    owner = persistLayoutLabEvacuation(owner, "W1N1", evacuation);
    owner = parseLayoutsOwner(JSON.parse(JSON.stringify(owner))) ?? emptyLayoutsOwner();

    const following = labMineralEvacuationFixture(750, 1_000, 10_000, 101, fixture.replacementId);
    const activeFollowing = activeReactionLabMigration(following);
    const logistics = projectLayoutLabEvacuations({
      existingBudgets: [],
      migrationRooms: [activeFollowing],
      records: owner.records,
      snapshot: { rooms: [following.room] } as unknown as WorldSnapshot,
      tick: 101,
    });
    const flowId = layoutLabEvacuationFlowId("W1N1", evacuation);
    if (flowId === null) throw new Error("expected active-reaction mineral flow identity");
    expect(logistics).toMatchObject({
      authorizedFlowIds: [flowId],
      budgets: [expect.objectContaining({ category: "optional-growth" })],
      demands: {
        edges: [expect.objectContaining({ id: flowId, maximumAmount: 750 })],
        endpoints: [
          expect.objectContaining({ resourceType: "XGH2O", targetId: "lab-external" }),
          expect.objectContaining({ resourceType: "XGH2O", targetId: "storage" }),
        ],
        nodes: [
          expect.objectContaining({ resourceType: "XGH2O" }),
          expect.objectContaining({
            capacityReservationKey: "store-capacity/4:W1N1/7:storage",
            resourceType: "XGH2O",
          }),
        ],
        suppressedSourceTargetIds: ["lab-external"],
      },
    });
    expect(
      projectLayoutLabEvacuations({
    ...
    ).toMatchObject({ authorizedFlowIds: [flowId] });
    for (const blockedMigration of [
      {
        ...activeFollowing,
        assignmentHandoff: { ...activeFollowing.assignmentHandoff, status: "pending" as const },
    ...
    const storage = following.room.ownedStorages?.[0];
    if (storage === undefined) throw new Error("expected active-reaction storage");
    for (const blockedRoom of [
      { ...following.room, hostileCreeps: [{}] },
      { ...following.room, ownedStorages: [{ ...storage, id: "storage-drift" }] },
      {
        ...following.room,
        ownedStorages: [{ ...storage, store: { ...storage.store, freeCapacity: 0 } }],
      },
    ])
      expect(
        projectLayoutLabEvacuations({
          ...
          migrationRooms: [activeFollowing],
          records: owner.records,
          snapshot: { rooms: [blockedRoom] } as unknown as WorldSnapshot,
          tick: 101,
        }),
      ).toMatchObject({ authorizedFlowIds: [], budgets: [], demands: { edges: [] } });
    const expired = labMineralEvacuationFixture(750, 1_000, 10_000, 250, fixture.replacementId);
    expect(
      projectLayoutLabEvacuations({
        existingBudgets: [],
        migrationRooms: [activeReactionLabMigration(expired)],
        records: owner.records,
        snapshot: { rooms: [expired.room] } as unknown as WorldSnapshot,
        tick: 250,
      }),
    ).toMatchObject({ authorizedFlowIds: [], budgets: [], demands: { edges: [] } });

    const partial = labMineralEvacuationFixture(300, 1_450, 10_450, 102, fixture.replacementId);
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set([flowId]),
        activeLogisticsTargetIds: new Set(["lab-external", "storage"]),
        colony: partial.colony,
    ...
    const driftedTarget = {
      ...partial.room,
      ownedLabs: (partial.room.ownedLabs ?? []).map((lab) =>
        lab.id === "lab-external"
          ? {
              ...lab,
              mineralType: "Z",
              store: {
                ...lab.store,
                resources: [{ amount: lab.mineralAmount, resourceType: "Z" }],
              },
            }
          : lab,
      ),
    ...
    expect(
      planMigration({
        ...
        colony: partial.colony,
        labEvacuation: evacuation,
        labMigration: activeReactionLabMigration({
          labMigration: partial.labMigration,
          room: driftedTarget,
        }),
        logisticsEvidenceReady: true,
  ...
  it("keeps a real pending reaction mixed-terminal evacuation funded while blocking removal", () => {
    const seed = labHandoffFixtureWorld(100);
    const retained = (seed.ownedRooms[0]?.ownedLabs ?? []).filter(({ id }) => id !== "external");
    const retainedAssignment = assignLabCluster({
      labs: retained,
      layoutFingerprint: fingerprintLabLayout("W1N1", retained),
      limits: { maximumBoostLabs: 2, maximumLabsScanned: 10, maximumOutputLabs: 8 },
      roomName: "W1N1",
    }).assignment;
    if (retainedAssignment === null) throw new Error("expected retained reaction assignment");
    const replacementId = [
      ...retainedAssignment.reagentLabIds,
      ...retainedAssignment.productLabIds,
      ...retainedAssignment.boostLabIds,
    ].sort()[0];
    if (replacementId === undefined) throw new Error("expected reaction replacement lab");
    const objective = { ...labFixtureObjective("forward"), amount: 30 };
    const first = composeReactionHandoffFixture(
      reactionTerminalSnapshot(100, false, 0, 0, 0, replacementId, 1_000),
      objective,
    );
    const previous = first.policy.commitments[0];
    if (previous?.kind !== "reaction") throw new Error("expected reaction commitment");

    const rebound = composeReactionHandoffFixture(
      reactionTerminalSnapshot(101, true, 100, 100, 0, replacementId, 1_000),
      objective,
    ...
    const readyWorld = reactionTerminalSnapshot(102, false, 100, 100, 0, replacementId, 1_000);
    const ready = composeReactionHandoffFixture(readyWorld, objective, durable, [], new Set());
    expect(ready.migrationRooms[0]?.assignmentHandoff).toMatchObject({
      kind: "reaction",
      status: "ready",
      targetLabId: "external",
    });
    expect(ready.migrationRooms[0]?.evacuationTerminalId).toBe("terminal");
    const intent = ready.intents[0];
    if (intent === undefined) throw new Error("expected retained reaction intent");
    const attempt = createPendingLabAttempt(intent, "OK");
    if (attempt === null) throw new Error("expected pending reaction attempt");
    ...
    const partialWorld = reactionTerminalSnapshot(103, true, 40, 60, 40, replacementId, 1_060);
    const partial = composeReactionHandoffFixture(
      partialWorld,
      objective,
      durable,
      [attempt],
    ...
    const completeWorld = reactionTerminalSnapshot(103, false, 0, 0, 100, replacementId, 1_100);
    const completeComposition = composeReactionHandoffFixture(
      completeWorld,
      objective,
      durable,
      [attempt],
    ...
    const completeMigration = completeComposition.migrationRooms[0];
    if (completeMigration === undefined) throw new Error("expected pending reaction migration");
    expect(completeMigration.activity).toContain("pending-attempt");
    const completeRoom = completeWorld.ownedRooms[0];
    if (completeRoom === undefined) throw new Error("expected complete reaction room");
    const room = {
      ...migrationRoom(),
      ...completeRoom,
      constructionSites: [],
      ...
      name: "W1N1",
      observedAt: 103,
      structures: (completeRoom.ownedLabs ?? []).map((value) =>
        structure(value.id, "lab", value.hits, value.hitsMax, value.pos.x, value.pos.y),
      ),
    } as unknown as RoomSnapshot;
    const placements = [
      ...retained.map(({ pos }) => ({
        ...placement("lab", pos.x, pos.y),
        adoption: "exact" as const,
        minimumRcl: 6,
      })),
      { ...placement("lab", 13, 12), minimumRcl: 6 },
    ];
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set(),
        activeLogisticsTargetIds: new Set(),
        colony: labTerminalEvacuationFixture(0, 100, 160, 103).colony,
        commitment: { ...migrationCommitment, fingerprint: "layout-commitment" },
        labEvacuation: evacuation,
        labMigration: completeMigration,
        logisticsEvidenceReady: true,
        placements,
  ...
  it("evacuates mixed stock atomically while a rebound reaction remains active", () => {
    ...
    const evacuation = staged.labEvacuation;
    if (evacuation === null || !("energyAmount" in evacuation))
      throw new Error("expected active mixed lab evacuation");
    let owner = persistLayoutCommitment(emptyLayoutsOwner(), "W1N1", migrationCommitment);
    owner = persistLayoutLabEvacuation(owner, "W1N1", evacuation);
    owner = parseLayoutsOwner(JSON.parse(JSON.stringify(owner))) ?? emptyLayoutsOwner();
    const persisted = owner.records[0]?.labEvacuation;
    if (persisted === undefined) throw new Error("expected persisted active mixed evacuation");
    const flowIds = layoutLabEvacuationFlowIds("W1N1", persisted);
    if (flowIds === null || flowIds.length !== 2)
      throw new Error("expected active mixed flow pair");
  ...
  it("evacuates mixed stock through an idle terminal while a rebound reaction remains active", () => {
    const fixture = labTerminalEvacuationFixture(750, 1_000, 10_000, 100, undefined, 500, 250);
    const staged = planMigration({
      activeLogisticsFlowIds: new Set(),
      activeLogisticsTargetIds: new Set(),
    ...
    const evacuation = staged.labEvacuation;
    if (evacuation === null || !("energyAmount" in evacuation))
      throw new Error("expected active mixed terminal evacuation");
    let owner = persistLayoutCommitment(emptyLayoutsOwner(), "W1N1", migrationCommitment);
    owner = persistLayoutLabEvacuation(owner, "W1N1", evacuation);
    owner = parseLayoutsOwner(JSON.parse(JSON.stringify(owner))) ?? emptyLayoutsOwner();
    const persisted = owner.records[0]?.labEvacuation;
    if (persisted === undefined) throw new Error("expected persisted active mixed terminal record");
    const flowIds = layoutLabEvacuationFlowIds("W1N1", persisted);
    if (flowIds === null || flowIds.length !== 2)
      throw new Error("expected active mixed terminal flow pair");
    ...
    expect(
      planMigration({
        ...
        labMigration: {
          ...completeMigration,
          activity: ["commitment", "intent", "pending-attempt"],
        },
        logisticsEvidenceReady: true,
        placements: completeFixture.placements,
        room: completeFixture.room,
  ...
  it("persists and completes one mineral-only quiescent-lab evacuation into active storage", () => {
    ...
    const evacuation = staged.labEvacuation;
    if (evacuation === null) throw new Error("expected mineral lab evacuation");
    let owner = persistLayoutCommitment(emptyLayoutsOwner(), "W1N1", migrationCommitment);
    owner = persistLayoutLabEvacuation(owner, "W1N1", evacuation);
    owner = parseLayoutsOwner(JSON.parse(JSON.stringify(owner))) ?? emptyLayoutsOwner();
    const persisted = owner.records[0]?.labEvacuation;
    if (persisted === undefined) throw new Error("expected persisted mineral evacuation");
    ...
    const flowId = layoutLabEvacuationFlowId("W1N1", persisted);
    if (flowId === null) throw new Error("expected bounded mineral flow identity");
    expect(logistics).toMatchObject({
      authorizedFlowIds: [flowId],
      demands: {
        edges: [expect.objectContaining({ id: flowId, maximumAmount: 750 })],
    ...
    if (complete.authorization === null) throw new Error("expected mineral removal authorization");
    const arbitration = arbitrateStructureRemovals({
      authorizations: [complete.authorization],
      limits: STRUCTURE_REMOVAL_LIMITS,
      proposals: complete.proposals,
    ...
    const consumed = labMineralEvacuationFixture(0, 1_749, 10_749, 103, fixture.replacementId);
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set(),
        activeLogisticsTargetIds: new Set(),
        colony: consumed.colony,
        labEvacuation: evacuation,
        labMigration: consumed.labMigration,
        logisticsEvidenceReady: true,
    ...
    const capacityLost = labMineralEvacuationFixture(750, 1_000, 999_500, 100);
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set(),
        activeLogisticsTargetIds: new Set(),
        colony: capacityLost.colony,
        labMigration: capacityLost.labMigration,
        logisticsEvidenceReady: true,
        placements: capacityLost.placements,
        room: capacityLost.room,
      }),
    ).toMatchObject({
      blockers: [expect.objectContaining({ reason: "evacuation-capacity" })],
      labEvacuation: null,
    ...
    const changedDestination = labMineralEvacuationFixture(
      0,
      1_750,
      10_750,
      103,
    ...
    const priorStorage = changedDestination.room.ownedStorages?.[0];
    if (priorStorage === undefined) throw new Error("expected mineral storage");
    const changedStorage = { ...priorStorage, id: "storage-new" };
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set(),
        activeLogisticsTargetIds: new Set(),
        colony: changedDestination.colony,
        labEvacuation: evacuation,
        labMigration: {
          ...changedDestination.labMigration,
          evacuationStorageId: changedStorage.id,
        },
        logisticsEvidenceReady: true,
        placements: changedDestination.placements,
        room: {
          ...changedDestination.room,
          ownedStorages: [changedStorage],
        },
      }).proposals,
    ).toEqual([]);

    const malformedStorage = labMineralEvacuationFixture(750, 1_000, 10_000, 100);
    const storage = malformedStorage.room.ownedStorages?.[0];
    if (storage === undefined) throw new Error("expected mineral storage");
    expect(
      planMigration({
        activeLogisticsFlowIds: new Set(),
        activeLogisticsTargetIds: new Set(),
        colony: malformedStorage.colony,
        labMigration: malformedStorage.labMigration,
        logisticsEvidenceReady: true,
        placements: malformedStorage.placements,
        room: {
          ...malformedStorage.room,
          ownedStorages: [
            {
              ...storage,
              store: {
                ...storage.store,
                resources: [...storage.store.resources, { amount: 1, resourceType: "XGH2O" }],
                usedCapacity: storage.store.usedCapacity + 1,
              },
            },
          ],
        },
    ...
    const expired = labMineralEvacuationFixture(
      100,
      1_650,
      10_650,
      evacuation.expiresAt,
      fixture.replacementId,
    );
    expect(
      planMigration({
...
function activeReactionLabMigration(fixture: {
...
function labMineralEvacuationFixture(
  ...
  const mineralType = targetMineral > 0 ? "XGH2O" : null;
  const ownedLabs = labs.map((value) =>
    value.id === target.id
      ? {
          ...value,
  ...
  const storageOther = storageUsed - storageMineral;
  if (storageOther < 0) throw new Error("storage total must include mineral baseline");
  const storage = {
    active: true,
    hits: 10_000,
    hitsMax: 10_000,
    ...
    store: {
      capacity: 1_000_000,
      freeCapacity: 1_000_000 - storageUsed,
      resources: [
        ...(storageOther === 0 ? [] : [{ amount: storageOther, resourceType: "energy" }]),
        ...(storageMineral === 0 ? [] : [{ amount: storageMineral, resourceType: "XGH2O" }]),
      ],
      usedCapacity: storageUsed,
    },
  } as const;
  return {
    ...base,
    labMigration: { ...base.labMigration, evacuationStorageId: storage.id },
    room: {
      ...base.room,
      ownedLabs,
...
function labTerminalEvacuationFixture(
  targetMineral: number,
...
function reactionTerminalSnapshot(
  tick: number,
  reversed: boolean,
  targetEnergy: number,
  targetMineral: number,
  ...
  if (room === undefined || storage === undefined)
    throw new Error("expected reaction terminal world");
    ...
    ownedRooms: [
      {
        ...
        ownedLabs: (room.ownedLabs ?? []).map((value) => {
          ...
          if (value.id !== replacementId) return value;
          return {
            ...value,
            energy: replacementEnergy,
            store: {
              ...value.store,
              freeCapacity: 5_000 - replacementEnergy - value.mineralAmount,
              resources: [
                { amount: replacementEnergy, resourceType: "energy" },
                ...value.store.resources.filter(({ resourceType }) => resourceType !== "energy"),
              ],
              usedCapacity: replacementEnergy + value.mineralAmount,
            },
          };
        }),
        ownedStorages: [],
