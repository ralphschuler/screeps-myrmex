...
export function emptyLayoutsOwner(): LayoutsOwnerV25 {
  return freeze({
...
export function parseLayoutsOwner(value: unknown): LayoutsOwnerV25 | null {
  if (
    !record(value) ||
    ...
    return null;
  const migratingBeforeV25 = value.schemaVersion < 25;
  if (
    (migratingBeforeV25 &&
      value.staleRecords !== undefined &&
      (!Array.isArray(value.staleRecords) || value.staleRecords.length !== 0)) ||
    (!migratingBeforeV25 && !Array.isArray(value.staleRecords))
  )
    return null;
  const rawStaleRecords = migratingBeforeV25 ? [] : (value.staleRecords as unknown[]);
  if (value.records.length + rawStaleRecords.length > MAX_LAYOUT_RECORDS) return null;
  const records: LayoutRecord[] = [];
  const staleRecords: StaleLayoutRecord[] = [];
  ...
  const migratingBeforeV20 = value.schemaVersion < 20;
  const migratingBeforeV21 = value.schemaVersion < 21;
  const migratingBeforeV22 = value.schemaVersion < 22;
  const migratingBeforeV23 = value.schemaVersion < 23;
  const migratingBeforeV24 = value.schemaVersion < 24;
  for (const rawItem of value.records) {
    ...
    if (migratingBeforeV18 && record(rawItem) && rawItem.terminalEvacuation !== undefined)
      return null;
    if (
      migratingBeforeV19 &&
      record(rawItem) &&
      ...
      return null;
    if (migratingBeforeV21 && record(rawItem) && rawItem.storageEvacuation !== undefined)
      return null;
    if (
      migratingBeforeV22 &&
      record(rawItem) &&
      record(rawItem.storageEvacuation) &&
      rawItem.storageEvacuation.resourceManifest !== undefined
    )
      return null;
    if (
      migratingBeforeV23 &&
      record(rawItem) &&
      record(rawItem.storageEvacuation) &&
      rawItem.storageEvacuation.settledAmount !== undefined
    )
      return null;
    if (
      migratingBeforeV24 &&
    ...
    if (
      ...
      record(rawItem) &&
      record(rawItem.removalReceipt) &&
      rawItem.removalReceipt.targetStructureType === "terminal"
    )
      return null;
    if (
      migratingBeforeV15 &&
    ...
    if (validStaleRecord(item)) {
      if (!migratingBeforeV25) return null;
  ...
  for (const item of rawStaleRecords) {
    if (!validStaleRecord(item)) return null;
    staleRecords.push(item);
  }
  records.sort((a, b) => compare(a.roomName, b.roomName));
  staleRecords.sort((a, b) => compare(a.roomName, b.roomName));
  const roomNames = [...records, ...staleRecords].map(({ roomName }) => roomName);
  if (new Set(roomNames).size !== roomNames.length) return null;
  return freeze({
    schemaVersion: LAYOUT_OWNER_SCHEMA_VERSION,
    revision: value.revision + (migratingBeforeV25 ? 1 : 0),
    records,
    staleRecords,
  });
}
...
export function persistLayoutCommitment(
  ...
  const staleRecords = owner.staleRecords.filter((record) => record.roomName !== roomName);
  ...
  records.push({
    ...
    ...(sameCommitment && prior.spawnEvacuation !== undefined
      ...
      : {}),
    ...(sameCommitment && prior.storageEvacuation !== undefined
      ? { storageEvacuation: prior.storageEvacuation }
      : {}),
    ...(sameCommitment && prior.terminalEvacuation !== undefined
      ? { terminalEvacuation: prior.terminalEvacuation }
      : {}),
  ...
  return freeze({
    ...
    records: records.slice(0, Math.max(0, MAX_LAYOUT_RECORDS - staleRecords.length)),
    staleRecords,
  });
}
...
export function persistLayoutStorageEvacuation(
  owner: LayoutsOwnerV25,
  roomName: string,
  evacuation: LayoutStorageEvacuation | null,
): LayoutsOwnerV25 {
  const prior = owner.records.find((record) => record.roomName === roomName);
  if (prior === undefined) return owner;
  if (
    (evacuation === null && prior.storageEvacuation === undefined) ||
    (evacuation !== null && JSON.stringify(evacuation) === JSON.stringify(prior.storageEvacuation))
  )
    return owner;
  const records = owner.records.map((record) => {
    if (record.roomName !== roomName) return record;
    if (evacuation === null) {
      const { storageEvacuation, ...retained } = record;
      return storageEvacuation === undefined ? record : retained;
    }
    return { ...record, storageEvacuation: evacuation };
  });
  return freeze({ ...owner, records, revision: owner.revision + 1 });
}
...
export function persistLayoutTerminalEvacuation(
  ...
  if (prior === undefined) return owner;
  if (
    (evacuation === null && prior.terminalEvacuation === undefined) ||
    (evacuation !== null && JSON.stringify(evacuation) === JSON.stringify(prior.terminalEvacuation))
  )
    return owner;
  const records = owner.records.map((record) => {
    if (record.roomName !== roomName) return record;
    if (evacuation === null) {
      const { terminalEvacuation, ...retained } = record;
      return terminalEvacuation === undefined ? record : retained;
    }
    return { ...record, terminalEvacuation: evacuation };
  });
  return freeze({ ...owner, records, revision: owner.revision + 1 });
}
...
export function reconcileOwnedLayouts(
  ...
  const records = owner.records.filter((record) => owned.has(record.roomName));
  const staleRecords = owner.staleRecords.filter((record) => owned.has(record.roomName));
  return records.length === owner.records.length &&
    staleRecords.length === owner.staleRecords.length
    ? owner
    : freeze({ ...owner, revision: owner.revision + 1, records, staleRecords });
}
...
export function clearStaleLayoutExtensionEvacuation(
  ...
  const prior = owner.staleRecords.find((record) => record.roomName === roomName);
  if (prior?.extensionEvacuation === undefined) return owner;
  const staleRecords = owner.staleRecords.map((record) => {
    if (record.roomName !== roomName) return record;
    const { extensionEvacuation: _extensionEvacuation, ...retained } = record;
...
export function clearStaleLayoutLabEvacuation(
  owner: LayoutsOwnerV25,
  roomName: string,
): LayoutsOwnerV25 {
  const prior = owner.staleRecords.find((record) => record.roomName === roomName);
  if (prior?.labEvacuation === undefined) return owner;
  const staleRecords = owner.staleRecords.map((record) => {
    if (record.roomName !== roomName) return record;
    const { labEvacuation: _labEvacuation, ...retained } = record;
    void _labEvacuation;
    return retained;
  });
  return freeze({ ...owner, revision: owner.revision + 1, staleRecords });
...
export function clearStaleLayoutLinkEvacuation(
  owner: LayoutsOwnerV25,
  roomName: string,
): LayoutsOwnerV25 {
  const prior = owner.staleRecords.find((record) => record.roomName === roomName);
  if (prior?.linkEvacuation === undefined) return owner;
  const staleRecords = owner.staleRecords.map((record) => {
    if (record.roomName !== roomName) return record;
    const { linkEvacuation: _linkEvacuation, ...retained } = record;
...
export function clearStaleLayoutSpawnEvacuation(
  owner: LayoutsOwnerV25,
  roomName: string,
): LayoutsOwnerV25 {
  const prior = owner.staleRecords.find((record) => record.roomName === roomName);
  if (prior?.spawnEvacuation === undefined) return owner;
  const staleRecords = owner.staleRecords.map((record) => {
    if (record.roomName !== roomName) return record;
    const { spawnEvacuation: _spawnEvacuation, ...retained } = record;
...
export function persistStaleLayoutStorageEvacuation(
  owner: LayoutsOwnerV25,
  roomName: string,
  ...
  const prior = owner.staleRecords.find((record) => record.roomName === roomName);
  if (
    prior?.storageEvacuation === undefined ||
    JSON.stringify(prior.storageEvacuation) === JSON.stringify(evacuation)
  )
    return owner;
  const staleRecords = owner.staleRecords.map((record) =>
    record.roomName === roomName ? { ...record, storageEvacuation: evacuation } : record,
  );
  return freeze({ ...owner, revision: owner.revision + 1, staleRecords });
}

export function clearStaleLayoutStorageEvacuation(
  owner: LayoutsOwnerV25,
  roomName: string,
): LayoutsOwnerV25 {
  const prior = owner.staleRecords.find((record) => record.roomName === roomName);
  if (prior?.storageEvacuation === undefined) return owner;
  const staleRecords = owner.staleRecords.map((record) => {
    if (record.roomName !== roomName) return record;
    const { storageEvacuation: _storageEvacuation, ...retained } = record;
...
export function clearStaleLayoutTowerEvacuation(
  ...
  const prior = owner.staleRecords.find((record) => record.roomName === roomName);
  if (prior?.towerEvacuation === undefined) return owner;
  const staleRecords = owner.staleRecords.map((record) => {
    if (record.roomName !== roomName) return record;
    const { towerEvacuation: _towerEvacuation, ...retained } = record;
...
export function clearStaleLayoutCompletedEvacuationReceipt(
  ...
  const staleRecords = owner.staleRecords.map((record) => {
    ...
    if (kind === "link") {
      ...
      void [_linkEvacuation, _removalReceipt];
      return retained;
    }
    if (kind === "spawn") {
      const {
    ...
    if (kind === "storage") {
      ...
      void [_removalReceipt, _storageEvacuation];
      return retained;
    }
    if (kind === "terminal") {
      const {
...
function validStaleRecord(v: unknown): v is StaleLayoutRecord {
  return validRecordShape(v, false);
}
function validRecordShape(v: unknown, currentAlgorithm: boolean): v is LayoutRecord {
  return (
    record(v) &&
    typeof v.algorithmRevision === "string" &&
    v.algorithmRevision.length > 0 &&
    ...
    (v.storageEvacuation === undefined || validStorageEvacuation(v.storageEvacuation)) &&
    (v.terminalEvacuation === undefined || validTerminalEvacuation(v.terminalEvacuation)) &&
...
function validStorageEvacuation(value: unknown): value is LayoutStorageEvacuation {
  if (
    !record(value) ||
    !integer(value.startedAt) ||
    !integer(value.expiresAt) ||
    !identity(value.sourceId, 128) ||
    !identity(value.terminalId, 128) ||
    value.sourceId === value.terminalId
  )
    return false;
  const hasManifest = "resourceManifest" in value;
  const hasScalar =
    "amount" in value || "resourceType" in value || "terminalInitialAmount" in value;
  if (hasManifest === hasScalar) return false;
  if (hasManifest) {
    const totals = storageResourceManifestTotals(value.resourceManifest);
    if (totals === null || totals.terminal + totals.amount > MAX_LAYOUT_TERMINAL_CAPACITY)
      return false;
    if (value.settledAmount === undefined)
      return (
        value.expiresAt - value.startedAt === LAYOUT_STORAGE_EVACUATION_TIMEOUT_TICKS &&
        totals.amount <= MAX_LAYOUT_STORAGE_EVACUATION_AMOUNT
      );
    return (
      value.expiresAt - value.startedAt === LAYOUT_STORAGE_SEQUENTIAL_EVACUATION_TIMEOUT_TICKS &&
      totals.amount > MAX_LAYOUT_STORAGE_EVACUATION_AMOUNT &&
      totals.amount <= MAX_LAYOUT_STORAGE_SEQUENTIAL_EVACUATION_AMOUNT &&
      (value.settledAmount === 0 || value.settledAmount === MAX_LAYOUT_STORAGE_EVACUATION_AMOUNT) &&
      value.settledAmount < totals.amount
    );
  }
  if (
    !positiveInteger(value.amount) ||
    !identity(value.resourceType, 64) ||
    value.resourceType !== value.resourceType.trim() ||
    !integer(value.terminalInitialAmount)
  )
    return false;
  if (value.settledAmount === undefined)
    return (
      value.expiresAt - value.startedAt === LAYOUT_STORAGE_EVACUATION_TIMEOUT_TICKS &&
      value.amount <= MAX_LAYOUT_STORAGE_EVACUATION_AMOUNT &&
      value.terminalInitialAmount + value.amount <= MAX_LAYOUT_TERMINAL_CAPACITY
    );
  return (
    value.expiresAt - value.startedAt === LAYOUT_STORAGE_SEQUENTIAL_EVACUATION_TIMEOUT_TICKS &&
    value.amount > MAX_LAYOUT_STORAGE_EVACUATION_AMOUNT &&
    value.amount <= MAX_LAYOUT_STORAGE_SEQUENTIAL_EVACUATION_AMOUNT &&
    (value.settledAmount === 0 || value.settledAmount === MAX_LAYOUT_STORAGE_EVACUATION_AMOUNT) &&
    value.settledAmount < value.amount &&
    value.terminalInitialAmount + value.amount <= MAX_LAYOUT_TERMINAL_CAPACITY
  );
}

function storageResourceManifestTotals(
  value: unknown,
): { readonly amount: number; readonly terminal: number } | null {
  if (
    !Array.isArray(value) ||
    value.length < 2 ||
    value.length > MAX_LAYOUT_STORAGE_EVACUATION_RESOURCES
  )
    return null;
  let prior = "";
  let amount = 0;
...
function validTerminalEvacuation(value: unknown): value is LayoutTerminalEvacuation {
  if (
    !record(value) ||
    !integer(value.startedAt) ||
    !integer(value.expiresAt) ||
    value.expiresAt - value.startedAt !== LAYOUT_TERMINAL_EVACUATION_TIMEOUT_TICKS ||
    !identity(value.sourceId, 128) ||
    !identity(value.replacementId, 128) ||
    value.sourceId === value.replacementId
  )
    return false;
  const hasManifest = value.resourceManifest !== undefined;
  const hasScalar =
    value.amount !== undefined ||
    value.replacementInitialAmount !== undefined ||
    value.resourceType !== undefined;
  if (hasManifest === hasScalar) return false;
  if (hasManifest) return validTerminalResourceManifest(value.resourceManifest);
  return (
    positiveInteger(value.amount) &&
    value.amount <= MAX_LAYOUT_TERMINAL_EVACUATION_AMOUNT &&
    integer(value.replacementInitialAmount) &&
    value.replacementInitialAmount + value.amount <= MAX_LAYOUT_STORAGE_CAPACITY &&
    identity(value.resourceType, 64) &&
    value.resourceType === value.resourceType.trim()
  );
}

function validTerminalResourceManifest(value: unknown): boolean {
  if (
    !Array.isArray(value) ||
    value.length < 2 ||
    value.length > MAX_LAYOUT_TERMINAL_EVACUATION_RESOURCES
  )
    return false;
  let prior = "";
  let amountTotal = 0;
  ...
  return (
    amountTotal <= MAX_LAYOUT_TERMINAL_EVACUATION_AMOUNT &&
