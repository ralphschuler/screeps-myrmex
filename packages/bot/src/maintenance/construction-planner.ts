  ...
  planMigration(input: {
    ...
    const candidates = [
      ...
      const leftActive =
        ...
        (left.kind === "storage" && left.target.id === input.storageEvacuation?.sourceId) ||
      ...
      const rightActive =
        ...
        (right.kind === "storage" && right.target.id === input.storageEvacuation?.sourceId) ||
    ...
    let storageEvacuation = input.storageEvacuation ?? null;
    if (
      removalDisappeared &&
      input.removalReceipt?.targetStructureType === "storage" &&
      input.removalReceipt.targetId === storageEvacuation?.sourceId
    )
      storageEvacuation = null;
    ...
    for (const candidate of considered) {
      if (candidate.kind === "spawn") {
        if (spawnEvacuation !== null && spawnEvacuation.sourceId !== candidate.target.id) continue;
        if (
        ...
        const requiredReplacementId =
          spawnEvacuation?.replacementId ??
        ...
        const identity = { replacementId: spawn.replacement.id, sourceId: candidate.target.id };
        const flowId = layoutSpawnEvacuationFlowId(input.room.name, identity);
        if (spawnEvacuation === null && spawn.targetEnergy > 0) {
          if (
            input.activeLogisticsFlowIds === undefined ||
            input.activeLogisticsTargetIds === undefined ||
            input.activeLogisticsEndpoints === undefined ||
            flowId === null ||
            layoutSpawnEvacuationBudgetIssuer(input.room.name, identity) === null
          ) {
            pushMigrationBlocker(blockers, {
              reason: "logistics-unavailable",
              roomName: input.room.name,
          ...
          if (
            spawn.replacementEnergy + spawn.targetEnergy > MAX_LAYOUT_SPAWN_ENERGY ||
            spawn.replacement.store.freeCapacity !==
              MAX_LAYOUT_SPAWN_ENERGY - spawn.replacementEnergy ||
            spawn.replacement.store.freeCapacity < spawn.targetEnergy
          ) {
            pushMigrationBlocker(blockers, {
          ...
          spawnEvacuation = {
            amount: spawn.targetEnergy,
            expiresAt: input.room.observedAt + LAYOUT_SPAWN_EVACUATION_TIMEOUT_TICKS,
            replacementId: spawn.replacement.id,
            replacementInitialEnergy: spawn.replacementEnergy,
            sourceId: candidate.target.id,
            startedAt: input.room.observedAt,
          };
        }
        if (spawnEvacuation !== null) {
          const activeFlowId = layoutSpawnEvacuationFlowId(input.room.name, spawnEvacuation);
          const flowActive =
            activeFlowId !== null && input.activeLogisticsFlowIds?.has(activeFlowId) === true;
          ...
          if (
            ...
            reason = "logistics-unavailable";
          else if (input.room.observedAt >= spawnEvacuation.expiresAt) {
            reason = "evacuation-expired";
            if (spawn.targetEnergy > 0 && !flowActive) spawnEvacuation = null;
          } else if (
            spawn.targetEnergy > spawnEvacuation.amount ||
            (spawn.targetEnergy > 0 &&
              spawn.replacementEnergy < spawnEvacuation.replacementInitialEnergy) ||
            (spawn.targetEnergy > 0 &&
              spawn.targetEnergy + spawn.replacementEnergy >
        ...
        admitRemoval({
          ...
          ...(spawnEvacuation === null
            ? {}
            : {
                replacementMinimumEnergy:
                  spawnEvacuation.replacementInitialEnergy + spawnEvacuation.amount,
              }),
          replacementRequiresIdle: true,
          replacementStructureType: "spawn",
          stableId: [
            spawnEvacuation === null ? "remove-spawn-v1" : "remove-spawn-v2",
            input.colony.id,
      ...
      if (candidate.kind === "storage") {
        if (storageEvacuation !== null && storageEvacuation.sourceId !== candidate.target.id)
          continue;
        if (
          input.logisticsEvidenceReady !== true ||
          input.activeLeasedWorkTargetIds === undefined ||
        ...
        const terminalBoundLabEvacuation =
          ...
          input.labEvacuation !== undefined &&
          "destinationId" in input.labEvacuation &&
          input.labEvacuation.destinationId === storage.terminal.id;
        const inverseTerminalEvacuation =
          input.terminalEvacuation !== null &&
          input.terminalEvacuation !== undefined &&
          input.terminalEvacuation.sourceId === storage.terminal.id &&
          input.terminalEvacuation.replacementId === candidate.target.id;
        if (terminalBoundLabEvacuation || inverseTerminalEvacuation) {
          pushMigrationBlocker(blockers, {
            reason: "logistics-active",
            roomName: input.room.name,
            targetId: candidate.target.id,
        ...
        if (storageEvacuation === null && storage.targetAmount > 0) {
          ...
          if (
            hasUnrelatedMigrationEndpoint(
              input,
              { replacementId: nextEvacuation.terminalId, sourceId: nextEvacuation.sourceId },
              null,
          ...
          storageEvacuation = nextEvacuation;
          pushMigrationBlocker(blockers, {
            reason: "target-stocked",
            roomName: input.room.name,
            targetId: candidate.target.id,
        ...
        if (storageEvacuation !== null) {
          ...
          const endpointsRetired =
            ...
            !input.activeLogisticsTargetIds.has(storageEvacuation.sourceId) &&
            !input.activeLogisticsTargetIds.has(storageEvacuation.terminalId) &&
            !input.activeLogisticsEndpoints.some(
              ({ counterpartId, targetId }) =>
                targetId === storageEvacuation?.sourceId ||
                targetId === storageEvacuation?.terminalId ||
                counterpartId === storageEvacuation?.sourceId ||
        ...
        admitRemoval({
          ...
          stableId: [
            storageEvacuation === null ? "remove-storage-v1" : "remove-storage-v2",
            input.colony.id,
            input.commitment.fingerprint,
            candidate.target.id,
            storage.terminal.id,
      ...
      if (candidate.kind === "terminal") {
        if (terminalEvacuation !== null && terminalEvacuation.sourceId !== candidate.target.id)
          continue;
        if (storageEvacuation?.terminalId === candidate.target.id) {
          pushMigrationBlocker(blockers, {
            reason: "logistics-active",
            roomName: input.room.name,
            targetId: candidate.target.id,
...
function spawnMigrationEvidence(
  ...
  | {
      readonly reason: null;
      readonly replacement: RoomSnapshot["ownedSpawns"][number];
      readonly replacementEnergy: number;
      readonly targetEnergy: number;
    } {
  const occupying = (room.structures ?? []).filter(({ pos }) => samePosition(pos, target.pos));
  ...
  const targetEnergy = observedTarget === undefined ? null : exactSpawnEnergy(observedTarget);
  if (
  ...
  if (replacement === undefined) return { reason: "spawn-selected", replacement: null };
  const replacementEnergy = exactSpawnEnergy(replacement);
...
function hasUnrelatedMigrationEndpoint(
  ...
  const related = (input.activeLogisticsEndpoints ?? []).filter(
    ({ counterpartId, targetId }) =>
      migrationIds.has(targetId) || (counterpartId !== null && migrationIds.has(counterpartId)),
  );
  if (
    related.some(
      ({ counterpartId, flowId, targetId, version }) =>
        ...
        !migrationIds.has(counterpartId),
    )
  )
    return true;
  const exactEndpointIds = new Set(
    related.flatMap(({ counterpartId, targetId }) =>
      counterpartId === null ? [] : [targetId, counterpartId],
    ),
  );
...
function storageMigrationEvidence(input: {
  ...
  readonly reason: Extract<
    LayoutMigrationBlocker,
    "replacement-pending" | "site-conflict" | "target-unavailable"
  > | null;
  readonly targetAmount: number;
  readonly targetResources: readonly (readonly [resourceType: string, amount: number])[];
  readonly terminal: NonNullable<RoomSnapshot["ownedTerminals"]>[number] | null;
  readonly terminalFreeCapacity: number;
  readonly terminalResources: readonly (readonly [resourceType: string, amount: number])[];
} {
  const failure = (reason: "replacement-pending" | "site-conflict" | "target-unavailable") => ({
    reason,
    targetAmount: 0,
    targetResources: [],
    terminal: null,
  ...
  const terminalStore =
    terminal === undefined ? null : exactInventoryStore(terminal, MAX_LAYOUT_TERMINAL_CAPACITY);
  if (
    terminal === undefined ||
    !terminal.active ||
  ...
  return {
    ...
    targetResources: [...targetStore.resources].sort(([left], [right]) => compareText(left, right)),
    terminal,
    terminalFreeCapacity: terminalStore.freeCapacity,
    terminalResources: [...terminalStore.resources].sort(([left], [right]) =>
