...
import {
  ...
  clearStaleLayoutStorageEvacuation,
  ...
  persistStaleLayoutStorageEvacuation,
  ...
  staleLayoutLabEvacuationSettlementBlocker,
...
import {
  ...
  projectLayoutExtensionEvacuationSuppressedSinkTargetIds,
  type LayoutExtensionEvacuationProjection,
} from "../logistics/extension-evacuation";
import {
...
import {
  ...
  type LayoutSpawnEvacuationProjection,
...
function composeRuntimeSystems(input: CompositionInput): readonly TickSystem<TickContext>[] {
  ...
  let activeStorageEvacuationFlowIds: ReadonlySet<string> = persistedLayoutStorageEvacuationFlowIds(
    input.manager,
  );
  let authorizedStorageEvacuationFlowIds: ReadonlySet<string> = new Set();
  let suppressedStorageEvacuationTargetIds: ReadonlySet<string> =
    persistedLayoutStorageEvacuationTargetIds(input.manager, input.runtime.context.tick);
  ...
  return Object.freeze([
    ...
    colonyDirectorSystem(
      ...
      (
        ...
        activeStorageEvacuations,
        authorizedStorageEvacuations,
        ...
        activeStorageEvacuationFlowIds = activeStorageEvacuations;
        authorizedStorageEvacuationFlowIds = authorizedStorageEvacuations;
        suppressedStorageEvacuationTargetIds = suppressedStorageEvacuationTargets;
    ...
    ),
    {
      ...
      run: ({ context }) => {
          ...
          if (
            ...
            (request.execution?.version !== 3 ||
              isAuthorizedLayoutStorageEvacuationFlowId(
                ...
                activeStorageEvacuationFlowIds,
        ...
        const retiringContractIds = new Set(
          logisticsRuntime.contracts.retirements.map(({ contractId }) => contractId),
        );
        ...
        for (const contract of context.contractPlanning.contracts) {
          ...
          if (
            contract.execution.version === 3 &&
            isLayoutStorageEvacuationFlowId(contract.execution.flowId)
          ) {
            if (!activeStorageEvacuationFlowIds.has(contract.execution.flowId)) {
              scope.producer.transition({
                contractId: contract.contractId,
                reason: "layout-storage-evacuation-orphaned",
                tick: context.tick,
                to: orphanedSpawnEvacuationTransition(contract.state),
              });
            } else if (
              !authorizedStorageEvacuationFlowIds.has(contract.execution.flowId) &&
              ...
              scope.producer.transition({
                contractId: contract.contractId,
                reason: "layout-storage-evacuation-not-authorized",
                tick: context.tick,
                to: "suspended",
              });
            } else if (
    ...
    },
    {
      ...
      run: ({ context }) => {
        ...
        for (const replacement of growth.replacements) scope.producer.replace(replacement);
    ...
    },
    {
      ...
      run: ({ context, budget }) => {
        ...
        const blockedStorageEvacuationFlowIds = context.contractExecution.leases.flatMap(
          ({ execution }) =>
            execution.version === 3 &&
            isLayoutStorageEvacuationFlowId(execution.flowId) &&
            !authorizedStorageEvacuationFlowIds.has(execution.flowId)
              ? [execution.flowId]
              : [],
        );
...
export function isLayoutLogisticsEvidenceReady(input: {
  readonly executionStatus: ContractExecutionView["status"];
  readonly gateEnabled: boolean;
  readonly health: LogisticsRuntimeProjection["health"];
  readonly observedAt: number;
  readonly planningStatus: ContractPlanningView["status"];
  readonly roomName: string;
}): boolean {
  if (!input.gateEnabled || input.executionStatus !== "ready" || input.planningStatus !== "ready")
    return false;
  const roomHealth = input.health.filter(({ colonyId }) => colonyId === input.roomName);
  return (
    roomHealth.length === 1 &&
    roomHealth[0]?.observedAt === input.observedAt &&
    roomHealth[0].status === "healthy"
  );
}
    ...
    run: ({ context }) => {
      ...
      const completedStaleExtensionEvacuationRooms = new Set(
        completedLayoutExtensionEvacuationRoomNames({
          activeFlowIds: activeLogisticsFlowIds,
          activeTargetIds: currentLogisticsTargetIds,
          records: initialOwner.staleRecords.filter(isStaleLayoutExtensionEvacuationContinuation),
          snapshot: context.snapshot,
          tick: context.tick,
        }),
      );
      ...
      const staleStorageEvacuationRecords = initialOwner.staleRecords.filter(
        isStaleLayoutStorageEvacuationContinuation,
      );
      const staleStorageEvacuationSettlements = new Map(
        projectLayoutStorageEvacuationSettlements({
          activeFlowIds: activeLogisticsFlowIds,
          activeTargetIds: activeTerminalLogisticsTargetIds,
          quiescentTerminalRoomNames,
          records: staleStorageEvacuationRecords,
          snapshot: context.snapshot,
          tick: context.tick,
        }).map((settlement) => [settlement.roomName, settlement]),
      );
      const staleLabEvacuationRecords = initialOwner.staleRecords.filter(
        isStaleLayoutLabEvacuationContinuation,
      );
      const completedStaleLabEvacuationRooms =
        staleLabEvacuationRecords.length === 0
          ? new Set<string>()
          : new Set(
              completedLayoutLabEvacuationRoomNames({
                activeFlowIds: activeLogisticsFlowIds,
                activeTargetIds: currentLogisticsTargetIds,
                migrationRooms: currentLabs().migrationRooms,
                records: staleLabEvacuationRecords,
                snapshot: context.snapshot,
                tick: context.tick,
              }),
            );
      const selectedSpawnIds =
        context.spawn.status === "planned" && context.spawn.broker !== null
          ? projectActiveSpawnClaimIds(context.spawn.broker)
          : null;
      const completedStaleSpawnEvacuationRooms = new Set(
        selectedSpawnIds === null
          ? []
          : completedLayoutSpawnEvacuationRoomNames({
              activeFlowIds: activeLogisticsFlowIds,
              activeTargetIds: currentLogisticsTargetIds,
              records: initialOwner.staleRecords.filter(isStaleLayoutSpawnEvacuationContinuation),
              selectedSpawnIds,
              snapshot: context.snapshot,
              tick: context.tick,
            }),
      ...
      for (const colony of planningColonies) {
        ...
        if (
          staleRecord !== undefined &&
          completedStaleLabEvacuationRooms.has(room.name) &&
          staleLayoutLabEvacuationSettlementBlocker({ colony, record: staleRecord }) === null
        ) {
          owner = clearStaleLayoutLabEvacuation(owner, room.name);
          changed = true;
          ownerPrecommitRequired = true;
          planning.push({
            blocker: "revision-handoff-active",
        ...
        const staleStorageEvacuationSettlement = staleStorageEvacuationSettlements.get(room.name);
        if (
          staleRecord !== undefined &&
          staleStorageEvacuationSettlement !== undefined &&
          staleLayoutStorageEvacuationSettlementBlocker({ colony, record: staleRecord }) === null
        ) {
          owner =
            staleStorageEvacuationSettlement.evacuation === null
              ? clearStaleLayoutStorageEvacuation(owner, room.name)
              : persistStaleLayoutStorageEvacuation(
                  owner,
                  room.name,
                  staleStorageEvacuationSettlement.evacuation,
                );
          changed = true;
          ownerPrecommitRequired = true;
          planning.push({
        ...
        if (
          ...
          owner = clearStaleLayoutSpawnEvacuation(owner, room.name);
          changed = true;
          ownerPrecommitRequired = true;
          planning.push({
            blocker: "revision-handoff-active",
        ...
        migrationInputs.push({
          ...
          logisticsEvidenceReady: isLayoutLogisticsEvidenceReady({
            executionStatus: context.contractExecution.status,
            gateEnabled: isFeatureEnabled(context.config, "phase2.logistics"),
            health: logistics.health,
            observedAt: room.observedAt,
            planningStatus: context.contractPlanning.status,
            roomName: room.name,
          }),
...
export function projectLayoutTerminalSendBlockedRoomNames(
  ...
  return new Set(
    records.flatMap(({ labEvacuation, roomName, storageEvacuation, terminalEvacuation }) =>
      (storageEvacuation !== undefined &&
        tick > storageEvacuation.startedAt &&
        tick < storageEvacuation.expiresAt) ||
      (terminalEvacuation !== undefined &&
        tick > terminalEvacuation.startedAt &&
        tick < terminalEvacuation.expiresAt) ||
      (labEvacuation !== undefined &&
        "destinationStructureType" in labEvacuation &&
        tick > labEvacuation.startedAt &&
        tick < labEvacuation.expiresAt)
        ? [roomName]
        : [],
    ),
  );
...
function layoutMigrationPlanningSystem(
  ...
  return {
    ...
    run: ({ context, budget }) => {
      ...
      const staleSpawnEvacuationFlowIds = new Set(
        initialOwner.staleRecords.flatMap((record) => {
          const evacuation = record.spawnEvacuation;
          if (evacuation === undefined || !isStaleLayoutSpawnEvacuationContinuation(record))
            return [];
          const flowId = layoutSpawnEvacuationFlowId(record.roomName, evacuation);
          return flowId === null ? [] : [flowId];
        }),
      );
      const authorizedSpawnEvacuations = new Set(
        [...currentAuthorizedSpawnEvacuations()].filter((flowId) =>
          staleSpawnEvacuationFlowIds.has(flowId),
        ),
      );
      ...
      for (const migrationInput of draft.migrationInputs) {
        ...
        migrationOwner = persistLayoutStorageEvacuation(
          migrationOwner,
          migrationInput.room.name,
          migration.storageEvacuation,
        );
  ...
  return Object.freeze({
    leases: Object.freeze(
      execution.leases.filter(({ execution: terms, targetId }) => {
        if (terms.version === 3 && isLayoutStorageEvacuationFlowId(terms.flowId))
          return allowedFlowIds.has(terms.flowId);
...
function colonyDirectorSystem(
  ...
  publishCandidates: (
    ...
    activeStorageEvacuations: ReadonlySet<string>,
    authorizedStorageEvacuations: ReadonlySet<string>,
    suppressedStorageEvacuationTargets: ReadonlySet<string>,
  ...
  return {
    ...
    run: ({ context, mode, budget }) => {
      ...
      const stalePolicyPlanningAdmitted =
        mode === "normal" &&
        (explicitStaleSourceService ||
          hasStaleLayoutContainerMigrationContinuation(staleLayoutsOwner) ||
          hasStaleLayoutExtensionEvacuationContinuation(staleLayoutsOwner) ||
          ...
          hasStaleLayoutStorageEvacuationContinuation(staleLayoutsOwner) ||
      ...
      if (stalePolicyPlanningAdmitted) {
        ...
        staleLayoutPolicyColonies = stalePolicy.result.colonies;
        if (explicitStaleSourceService)
          sourceServiceHandoff = planLayoutSourceServiceHandoff(
            input,
            context,
            staleLayoutPolicyColonies,
          );
      }
      ...
      const persistedLayoutsOwner = parseLayoutsOwner(input.manager?.ownerView("layouts") ?? null);
      const persistedLayoutRecords = persistedLayoutsOwner?.records ?? [];
      const layoutWorkEnabled =
        isFeatureEnabled(context.config, "phase2.layout") &&
      ...
      const staleExtensionEvacuationRecords = layoutWorkEnabled
        ...
        : [];
      const extensionEvacuationRecords = [
        ...layoutRecords,
        ...staleExtensionEvacuationRecords.filter((record) => {
          const colony = staleLayoutPolicyColonies.find(
            ({ roomName }) => roomName === record.roomName,
          );
          return (
      ...
      const knownStaleLabEvacuationRecords =
        persistedLayoutsOwner?.staleRecords.filter(isKnownV1StaleLayoutLabEvacuation) ?? [];
      const staleLabEvacuationRecords = knownStaleLabEvacuationRecords.filter(
        isStaleLayoutLabEvacuationContinuation,
      );
      const authorizedStaleLabEvacuationRecords = layoutWorkEnabled
        ? staleLabEvacuationRecords.filter((record) => {
            const colony = staleLayoutPolicyColonies.find(
              ({ roomName }) => roomName === record.roomName,
            );
            return (
              colony !== undefined &&
              staleLayoutLabEvacuationSettlementBlocker({ colony, record }) === null
            );
          })
        : [];
      const labEvacuationRecords = [...layoutRecords, ...authorizedStaleLabEvacuationRecords];
      ...
      const spawnEvacuationRecords = [
        ...layoutRecords,
        ...(layoutWorkEnabled
          ? staleSpawnEvacuationRecords.filter((record) => {
              const colony = staleLayoutPolicyColonies.find(
                ({ roomName }) => roomName === record.roomName,
              );
              return (
                colony !== undefined &&
                staleLayoutSpawnEvacuationSettlementBlocker({ colony, record }) === null
              );
            })
          : []),
      ];
      ...
      const persistedStaleStorageEvacuationRecords =
        persistedLayoutsOwner?.staleRecords.filter(isPersistedStaleLayoutStorageEvacuation) ?? [];
      const staleStorageEvacuationRecords = persistedStaleStorageEvacuationRecords.filter(
        isStaleLayoutStorageEvacuationContinuation,
      );
      ...
      const terminalSendBlockedRoomNames = projectLayoutTerminalSendBlockedRoomNames(
        [
          ...
          ...persistedStaleStorageEvacuationRecords,
      ...
      const terminalWork = isFeatureEnabled(context.config, "phase2.industry")
        ? projectIndustryTerminalWork({
            plan: industryProjection.plan,
            previous: industryDraft.states,
      ...
      const authorizedStaleStorageEvacuationRecords = layoutWorkEnabled
        ? staleStorageEvacuationRecords.filter((record) => {
            const colony = staleLayoutPolicyColonies.find(
              ({ roomName }) => roomName === record.roomName,
            );
            return (
              ...
              staleLayoutStorageEvacuationSettlementBlocker({ colony, record }) === null
            );
          })
        : [];
      const storageEvacuationRecords = [
        ...layoutRecords,
        ...authorizedStaleStorageEvacuationRecords,
      ];
      ...
      const extensionEvacuationSuppressedSinkTargetIds =
        projectLayoutExtensionEvacuationSuppressedSinkTargetIds({
          records: [...layoutRecords, ...staleExtensionEvacuationRecords],
          snapshot: context.snapshot,
          tick: context.tick,
        });
      const layoutEvacuations: LayoutExtensionEvacuationProjection = Object.freeze({
        budgets: projectedLayoutEvacuations.budgets,
        demands: Object.freeze({
          ...projectedLayoutEvacuations.demands,
          suppressedSinkTargetIds: extensionEvacuationSuppressedSinkTargetIds,
        }),
      });
      economyCandidates = withoutSuppressedSurvivalTransfers(
        economyCandidates,
      ...
      const spawnEvacuationSuppressedSinkTargetIds =
        projectLayoutSpawnEvacuationSuppressedSinkTargetIds({
          records: [...persistedLayoutRecords, ...staleSpawnEvacuationRecords],
          snapshot: context.snapshot,
          tick: context.tick,
        });
      const layoutSpawnEvacuations: LayoutSpawnEvacuationProjection = Object.freeze({
        budgets: projectedLayoutSpawnEvacuations.budgets,
        demands: Object.freeze({
          ...projectedLayoutSpawnEvacuations.demands,
          suppressedSinkTargetIds: spawnEvacuationSuppressedSinkTargetIds,
        }),
      });
      const suppressedSpawnEvacuationTargets = new Set(spawnEvacuationSuppressedSinkTargetIds);
      economyCandidates = withoutSuppressedSurvivalTransfers(
        economyCandidates,
        suppressedSpawnEvacuationTargets,
      );
      const projectedLayoutStorageEvacuations = projectLayoutStorageEvacuations({
        existingBudgets: priorLedger,
        includeWork: layoutWorkEnabled,
        quiescentTerminalRoomNames,
        records: storageEvacuationRecords,
        snapshot: context.snapshot,
        tick: context.tick,
      });
      const layoutStorageEvacuationSuppression = projectLayoutStorageEvacuations({
        existingBudgets: priorLedger,
        includeWork: false,
        quiescentTerminalRoomNames,
        records: [...persistedLayoutRecords, ...persistedStaleStorageEvacuationRecords],
        snapshot: context.snapshot,
        tick: context.tick,
      });
      const storageSuppressedSinkTargetIds = [
        ...new Set([
          ...(projectedLayoutStorageEvacuations.demands.suppressedSinkTargetIds ?? []),
          ...(layoutStorageEvacuationSuppression.demands.suppressedSinkTargetIds ?? []),
        ]),
      ].sort();
      const storageSuppressedSourceTargetIds = [
        ...new Set([
          ...(projectedLayoutStorageEvacuations.demands.suppressedSourceTargetIds ?? []),
          ...(layoutStorageEvacuationSuppression.demands.suppressedSourceTargetIds ?? []),
        ]),
      ].sort();
      const layoutStorageEvacuations = Object.freeze({
        ...projectedLayoutStorageEvacuations,
        demands: Object.freeze({
          ...projectedLayoutStorageEvacuations.demands,
          suppressedSinkTargetIds: Object.freeze(storageSuppressedSinkTargetIds),
          suppressedSourceTargetIds: Object.freeze(storageSuppressedSourceTargetIds),
        }),
      });
      const suppressedStorageEvacuationTargets = new Set([
        ...storageSuppressedSinkTargetIds,
        ...storageSuppressedSourceTargetIds,
      ]);
      economyCandidates = withoutSuppressedSurvivalTransfers(
        economyCandidates,
        suppressedStorageEvacuationTargets,
            ...
            committedLabLayouts:
              ...
              context.snapshot.ownedRooms.some(({ controller }) => controller.level === 8)
                ? [
                    ...
                    ...(authorizedStaleLabEvacuationRecords.length === 0
                      ? []
                      : projectStaleLabEvacuationLayouts(
                          context.snapshot,
                          authorizedStaleLabEvacuationRecords,
                        )),
                  ]
                : [],
      ...
      const projectedLayoutLabEvacuations = projectLayoutLabEvacuations({
        existingBudgets: priorLedger,
        migrationRooms: labs.migrationRooms,
        records: labEvacuationRecords,
        snapshot: context.snapshot,
        tick: context.tick,
      });
      const layoutLabEvacuationSuppression =
        layoutWorkEnabled && knownStaleLabEvacuationRecords.length === 0
          ? Object.freeze({
              suppressedSinkTargetIds:
                projectedLayoutLabEvacuations.demands.suppressedSinkTargetIds,
              suppressedSourceTargetIds:
                projectedLayoutLabEvacuations.demands.suppressedSourceTargetIds,
            })
          : projectLayoutLabEvacuationSuppression({
              records: [...persistedLayoutRecords, ...knownStaleLabEvacuationRecords],
              snapshot: context.snapshot,
              tick: context.tick,
            });
      const layoutLabEvacuations = Object.freeze({
        ...projectedLayoutLabEvacuations,
        demands: Object.freeze({
          ...projectedLayoutLabEvacuations.demands,
      ...
      if (isFeatureEnabled(context.config, "phase2.logistics")) {
        const startedAt = input.game.cpu.getUsed();
        const mergedResourceDemands = mergeResourceDemands(
        ...
        logisticsPlan = planLogisticsRuntime({
          ...
          planning: context.contractPlanning,
          resourceDemands: withoutSuppressedResourceDemandTargets(
            mergedResourceDemands,
            suppressedStorageEvacuationTargets,
            new Set(layoutStorageEvacuations.demands.edges.map(({ id }) => id)),
          ),
          snapshot: context.snapshot,
          tick: context.tick,
        });
      ...
      const budgetRequests = [
        ...
        ...layoutStorageEvacuations.budgets.filter(({ issuer }) =>
          layoutStorageEvacuations.demands.edges.some(
            ({ budgetBinding, id }) =>
      ...
      const authorizedSpawnEvacuationFlows =
        brokerResult?.status !== "planned"
          ? new Set<string>()
          : authorizeLayoutSpawnEvacuationFlowIds(
              spawnEvacuationRecords,
              executableSpawnEvacuationFlowIds,
      ...
      const activeStorageEvacuationBudgetIssuers = new Set(
        session.result.reservations.flatMap(({ category, issuer, status }) =>
          category === "optional-growth" && status === "active" ? [issuer] : [],
        ),
      );
...
function persistedLayoutLabEvacuationFlowIds(manager: MemoryManager | null): ReadonlySet<string> {
  ...
  return new Set(
    [...owner.records, ...owner.staleRecords.filter(isKnownV1StaleLayoutLabEvacuation)].flatMap(
      ({ labEvacuation, roomName }) => {
...
function persistedLayoutSpawnEvacuationFlowIds(manager: MemoryManager | null): ReadonlySet<string> {
  ...
  return new Set(
    [
      ...owner.records,
      ...owner.staleRecords.filter(isStaleLayoutSpawnEvacuationContinuation),
    ].flatMap(({ roomName, spawnEvacuation }) => {
...
function persistedLayoutStorageEvacuationFlowIds(
  ...
  return new Set(
    [
      ...owner.records,
      ...owner.staleRecords.filter(isPersistedStaleLayoutStorageEvacuation),
    ].flatMap(({ roomName, storageEvacuation }) => {
...
export function isAuthorizedLayoutStorageEvacuationFlowId(
  flowId: string,
  persistedFlowIds: ReadonlySet<string>,
  authorizedFlowIds: ReadonlySet<string>,
): boolean {
  return (
    !isLayoutStorageEvacuationFlowId(flowId) ||
    (persistedFlowIds.has(flowId) && authorizedFlowIds.has(flowId))
  );
}
...
function persistedLayoutStorageEvacuationTargetIds(
  manager: MemoryManager | null,
  tick: number,
): ReadonlySet<string> {
  if (manager === null) return new Set();
  ...
  return new Set(
    [
      ...owner.records,
      ...owner.staleRecords.filter(isPersistedStaleLayoutStorageEvacuation),
    ].flatMap(({ storageEvacuation }) =>
      storageEvacuation === undefined ||
      tick <= storageEvacuation.startedAt ||
      tick >= storageEvacuation.expiresAt
        ? []
        : [storageEvacuation.sourceId, storageEvacuation.terminalId],
    ),
  );
}
...
function hasExplicitStaleSourceService(owner: LayoutsOwnerV25 | null): boolean {
  return (
    owner?.staleRecords.some(
      ({ sourceServices }) =>
...
function hasStaleLayoutContainerMigrationContinuation(owner: LayoutsOwnerV25 | null): boolean {
  return owner?.staleRecords.some(isStaleLayoutContainerMigrationContinuation) === true;
}

function hasStaleLayoutExtensionEvacuationContinuation(owner: LayoutsOwnerV25 | null): boolean {
  return owner?.staleRecords.some(isStaleLayoutExtensionEvacuationContinuation) === true;
}

function hasStaleLayoutLabEvacuationContinuation(owner: LayoutsOwnerV25 | null): boolean {
  return owner?.staleRecords.some(isStaleLayoutLabEvacuationContinuation) === true;
}

function hasStaleLayoutLinkEvacuationContinuation(owner: LayoutsOwnerV25 | null): boolean {
  return owner?.staleRecords.some(isStaleLayoutLinkEvacuationContinuation) === true;
}
...
function hasStaleLayoutStorageEvacuationContinuation(owner: LayoutsOwnerV25 | null): boolean {
  return owner?.staleRecords.some(isStaleLayoutStorageEvacuationContinuation) === true;
}
...
function hasStaleLayoutTowerEvacuationContinuation(owner: LayoutsOwnerV25 | null): boolean {
  return owner?.staleRecords.some(isStaleLayoutTowerEvacuationContinuation) === true;
}
...
function blockedStaleLayoutExtensionEvacuationFlowIds(
  ...
  return Object.freeze(
    owner.staleRecords.flatMap((record) => {
      if (!isStaleLayoutExtensionEvacuationContinuation(record)) return [];
      const evacuation = record.extensionEvacuation;
      if (evacuation === undefined) return [];
      const flowId = layoutExtensionEvacuationFlowId(record.roomName, evacuation);
...
/** Narrow known-V1 geometry used only by an already-authorized stale lab evacuation. */
export function projectStaleLabEvacuationLayouts(
  snapshot: WorldSnapshot,
  records: readonly LayoutRecord[],
): readonly CommittedLabLayout[] {
  const unlocks = COLONY_RCL_POLICY_TABLE.find(({ level }) => level === 8)?.unlocks;
  if (unlocks === undefined || records.length > 64 || snapshot.rooms.length > 64)
    return Object.freeze([]);
  const layouts = [...snapshot.rooms]
    .sort((left, right) => left.name.localeCompare(right.name))
    .flatMap((room): readonly CommittedLabLayout[] => {
      if (room.controller?.ownership !== "owned" || room.controller.level !== 8) return [];
      const roomRecords = records.filter(({ roomName }) => roomName === room.name);
      const record = roomRecords[0];
      if (
        roomRecords.length !== 1 ||
        record === undefined ||
        !isStaleLayoutLabEvacuationContinuation(record)
      )
        return [];
      const labPositions = reconstructStaleLayoutLabPlacements({
        commitment: commitmentFromRecord(record),
        roomName: room.name,
        unlocks,
      });
      return labPositions === null
        ? []
        : [
            Object.freeze({
              labPositions,
...
export function withoutSuppressedResourceDemandTargets(
  projection: LogisticsResourceDemandProjection,
  suppressedTargetIds: ReadonlySet<string>,
  exemptFlowIds: ReadonlySet<string>,
): LogisticsResourceDemandProjection {
  if (suppressedTargetIds.size === 0) return projection;
  const exemptNodeIds = new Set(
    projection.edges.flatMap(({ id, sinkNodeId, sourceNodeId }) =>
      exemptFlowIds.has(id) ? [sourceNodeId, sinkNodeId] : [],
    ),
  );
  const blockedNodeIds = new Set(
    projection.endpoints.flatMap(({ nodeId, targetId }) =>
      targetId !== null && suppressedTargetIds.has(targetId) && !exemptNodeIds.has(nodeId)
        ? [nodeId]
        : [],
    ),
  );
  const edges = projection.edges.filter(
    ({ id, sinkNodeId, sourceNodeId }) =>
      exemptFlowIds.has(id) ||
      (!blockedNodeIds.has(sourceNodeId) && !blockedNodeIds.has(sinkNodeId)),
  );
  const retainedNodeIds = new Set(
    edges.flatMap(({ sinkNodeId, sourceNodeId }) => [sourceNodeId, sinkNodeId]),
  );
  return Object.freeze({
    ...projection,
    edges: Object.freeze(edges),
    endpoints: Object.freeze(
      projection.endpoints.filter(({ nodeId }) => retainedNodeIds.has(nodeId)),
    ),
    nodes: Object.freeze(projection.nodes.filter(({ id }) => retainedNodeIds.has(id))),
  });
}
...
function mergeResourceDemands(
  ...
  return Object.freeze({
    ...
    suppressedSinkTargetIds: Object.freeze(
      [
        ...new Set(
          projections.flatMap(({ suppressedSinkTargetIds }) => suppressedSinkTargetIds ?? []),
        ),
    ...
    suppressedSourceTargetIds: Object.freeze(
      [
        ...new Set(
          projections.flatMap(({ suppressedSourceTargetIds }) => suppressedSourceTargetIds ?? []),
        ),
      ].sort((left, right) => left.localeCompare(right)),
    ),
  });
