...
The scheduled auto-respawn workflow is disabled until the `screeps-production` environment contains
a dedicated Screeps token, a shard, and `SCREEPS_AUTO_RESPAWN_ENABLED=true`. It reads account
health, acts only on recognized terminal states, selects a viable start-room tile, verifies
placement, and keeps target coordinates out of workflow logs.
